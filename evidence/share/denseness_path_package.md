...
- **15.194 row mass:** sufficient lemma \(N_e<4-6/p\Rightarrow\) dual-eq empty (proved algebra). Pure \(N_e\) target **fails** Paley at \(p=3,5,7\). Refined row+mass LP max \(\kappa_e\approx1.19,1.17<\) need at \(p=5,7\) (census only). Preferred general target: Max+-free row+mass \(\max\kappa_e<2-\alpha\).
