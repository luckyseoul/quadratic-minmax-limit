...
- **15.197–198 K₄ path:** \(d_H\ge p+1\) proved; \(Q=K_4/(n(n-1))-2(3n-2)/(n-1)\); thr \(K_4\le16n^2-14n\). **Proved** \(\mathrm{Wick}_{hi}=12n^2+48n\le\) thr for \(p\ge5\), and \(K_4\le\mathrm{Wick}_{hi}\Rightarrow Q\le6+58/(n-1)<10\). Partition \(K_4=12n^2-48n+C_{\mathrm{diag}}+24\sum\eta_S^2\). Census \(K_4\le\mathrm{Wick}_{hi}\) at p=3(=),5,7. **OPEN:** Max+-free \(K_4\le\mathrm{Wick}_{hi}\) (same open scalar as 15.95 \(\sum M^2\)).
