...
- **15.197–199 K₄ path:** \(d_H\ge p+1\) proved; \(Q=K_4/(n(n-1))-2(3n-2)/(n-1)\); thr \(K_4\le16n^2-14n\). **Proved** \(\mathrm{Wick}_{hi}=12n^2+48n\le\) thr for \(p\ge5\), and \(K_4\le\mathrm{Wick}_{hi}\Rightarrow Q\le6+58/(n-1)<10\). Partition + frame identity \(K_4=4n^2+E[A^2]\). λ_max-only Wick path **dead** at p=5. Census \(K_4\le\mathrm{Wick}_{hi}\) at p=3(=),5,7. **OPEN:** Max+-free \(K_4\le\mathrm{Wick}_{hi}\) (same open scalar as 15.95 \(\sum M^2\)).
