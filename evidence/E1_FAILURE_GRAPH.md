...
| **F25** | Apply ordinary Delsarte/external-distance or a fixed-\(L^q\) moment-norm lower bound to the augmented cut code | The dual has broad even-Eulerian weight support, while every fixed \(L^q\) norm of a quadratic form is only \(O_q(n)\). Both mechanisms lose the required \(n^{3/2}\) term. | Preserve multiplicative phases across many Eulerian shells; the exact replacement target is the signed free-energy bound (9) in `NOTE_2026-08-29_global_minimality_and_local_stability_no_go.md`. |
