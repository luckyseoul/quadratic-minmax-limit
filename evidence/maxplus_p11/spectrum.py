...
q=p*p;n=q+1
if p==11:
    Yfull=np.load('/tmp/e1work/maxplus_p11_eps1.npy')  # keep int8; float64 cast of
                                                        # the full 37.4M x 122 array
                                                        # is 34GB and OOMs even at 60GB
else:
...
# chunked projection: cast only one chunk to float64 at a time
Z61=np.empty((Nh,d),dtype=np.float64)
_step=500_000
for _lo in range(0,Nh,_step):
    Z61[_lo:_lo+_step]=Yfull[_lo:_lo+_step].astype(np.float64)@Vp
# svec index
