...
The staged engine was then rerun through the already covered interval below
five million, this time retaining every two-gate collapse rather than only
one-sided full triples.  The ECC V100 covered `29..3000000` and NUKA's HIP
worker covered `3000001..5000000` concurrently.  All 14,569 eligible primes
completed in about 209 seconds of wall time with no counterexample.  The
complete deep-prefix list below five million is

    133253, 1056917, 1638701, 1740173, 1965629,
    3107441, 3899309, 4374833, 4746701.

Five of these records were invisible in the previous summaries because they
were neither one-sided full triples nor part of the small full census.  With
the 5--10 million extension, the exact staged dataset now contains 27,701
eligible primes, 522 first-gate stalls, 20 two-gate collapses, and no
three-gate collapse.  The complete lower certificates are
`evidence/w2_d21_boundary_gpu_staged_29_3000000.json` and
`evidence/w2_d21_boundary_gpu_staged_3000001_5000000.json`.  Individual OEIS
searches on the five newly exposed primes found one unrelated coincidence:
`1965629` begins one six-prime constellation in the tables attached to
[A022004](https://oeis.org/A022004/a022004_2.pdf).

The Wieferich-style atomic approach also makes a richer local lift cheap.
`scripts/w2_d21_boundary_mod4_gpu.py` replaces the final atomic XOR by an
exact atomic addition, retains each nonsquare order-21 class count modulo
four, and reduces the result in `(Z/4Z)[X]/(Phi_21)`.  The shared ring
arithmetic is `src/w2_d21_mod4_ring.py`.  At `p=133253`, all three GPU vectors
agree with the independent native CPU lift under the expected character
coordinate change `X -> X^4` and common shift `X^7` (equivalently `X^11` by
Frobenius symmetry).  A serialized NUKA lift of all 522 first-gate stalls took
69.50 seconds and reproduced the exact full patterns

    [true,false,false]: 494,
    [true,false,true]:    8,
    [true,true,false]:   20.

There are no triple zeros.  Whenever a difference vanishes modulo two, its
coefficientwise half-lift modulo two projects into the `F_8` subfield at both
reciprocal sextics; the census has no violation of this sharper local
placement.  It does not yet close W2.  All 64 first-half-lift state pairs
occur, and 15 contain both gate-two outcomes.  On each single sextic every
one of the 64 `(half-lift, next parity)` inputs has multiple gate-three
outputs on one side (63 of 64 on the other), with as many as all eight `F_8`
values.  A six-term Frobenius-linear recurrence is inconsistent on both
sides.  Even after reciprocal coupling, 43 of 477 observed inputs have two or
three outputs.  The 20 decisive third pairs have full linear and affine rank
six over `F_2`.

Support-bit fitting gives another clean holdout rejection.  Below five
million, the 27 observed six-bit support patterns have one quadratic
relation.  The disjoint 5--10 million holdout raises the combined quadratic
evaluation rank to the full 22 and kills it.  Cubics can separate the four
hypothetical counterexample support states only because 42 features
interpolate 33 observed patterns; nine such unproved fits remain after the
holdout.  This is not evidence for a theorem.  The full atomic lift and its
compact reproducible audit are
`evidence/w2_d21_boundary_mod4_gpu_first_gate_through_10000000.json` and
`evidence/w2_d21_mod4_state_analysis_through_10000000.json`, generated by
`scripts/w2_d21_mod4_state_analysis.py`.
