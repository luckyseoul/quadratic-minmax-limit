...
## 7bn. Global positive-p7 z7 reduction and Johnson semigroup (2026-08-30)

Proposition 15.718 begins with the 56 positive `z=7` line boundaries in two
source orbits.  Their exact-mean and pointed-branch expansion has 4,320 cases.
An exact affine-hull sieve rejects 3,024 and leaves 1,296; exact affine
symmetry partitions those survivors into 324 complete four-case classes.  A
joint same-catalog-row mod-3/mod-7 global join then gives 87 rigorous empty
joins, 159 necessary-only survivors, and 78 explicit budget skips among the
representatives.  Symmetry transfers those outcomes to 348 pointed
rejections, 636 necessary survivors, and 312 skips.  The 348 are pointed
leaf-case exclusions, not source-boundary exclusions, so the actual count
remains 56.

The independent Johnson-incidence calculation identifies the complete
uncapped semigroup.  Its 896 binary Hilbert-basis rows split by generator
grade as `56/168/672` in grades `1/2/3`; there is no primitive generator above
grade three.  Its exact layer counts for grades zero through eight are

```text
1, 56, 1,764, 37,856, 575,407, 6,496,938, 57,232,105,
410,200,367, 2,474,264,653.
```

Binarity makes the coordinate cap automatic through grade six.  The grade-
eight layer is an exact uncapped semigroup count but only an outer catalog
count unless `L_X<=6` is enforced explicitly.  This closes high-catalog
structure, not case or binary-edge feasibility.

The full proof and artifact manifest are
`evidence/NOTE_2026-08-30_p7_infinity7_positive_z7_global_semigroup.md`; the
hash-pinned compact certificate is
`evidence/p7_infinity7_positive_z7_global_semigroup_summary.json`.  Targeted
GitHub-code, MathOverflow, literature, and OEIS searches found surrounding
Johnson-scheme, Hilbert-basis, and finite-geometry material, but no prior
occurrence of this exact p7 census or joint modular reduction.  Numerical
matches have no evidentiary role.

## 7bo. Finite projected-semigroup stabilization (2026-08-30)

Proposition 15.719 turns the maximum Hilbert-generator grade three into a
finite stabilization theorem.  If four consecutive projected recurrence
layers agree, finite translation injectivity makes the common support the
subgroup generated by all projected Hilbert generators, and every later
uncapped layer is identical.  The complete `k=3` and `k=4` artifacts have
equal raw and anchor-relative supports on grades 3--6 in all eight
directions.  Their subgroup orders on direction blocks `0--4/5/6--7` are
respectively `147/3/3` and `1,029/21/3`.  These projections are cap-exact
through grade six and outer only at grade eight.

Two negative computations are retained solely as attack history.  Exact
`H0_S0_M7` torsion projections from `k=2` through `k=6` were nonseparating.
Bounded case joins processed all 51 grade-three-only representatives under
eight selected `k=3` projections and completed one-projection `k=4` and `k=5`
prefixes, with no rejection or skip.  An even 26-case `k=6` shard also found
no rejection; its odd mate was cancelled, so the `k=6` campaign is
incomplete.  One direct fixed-case model then imposed the full
`F_3^6 x F_7^21` quotient and returned `UNKNOWN` after 300 seconds.  That
status has no mathematical force.  The semigroup/quotient route was
terminated rather than extended.  Target presence in either family is only a
necessary condition and does not certify feasibility.

The compact certificate is
`evidence/p7_infinity7_positive_z7_projected_stabilization_summary.json`.
