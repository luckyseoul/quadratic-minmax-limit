...
| Residual (ii), even \(k\ge4p\) | **OPEN** | multi-level Max− leftover (15.274); p=5 \(k=20\) leftover+splus empty all nF (15.521/524/528); p=5 \(k=22\) leftover+splus empty nF=0,3–8,14 (15.547) |
| Type I, multi-level Max− | **OPEN** | 15.543 \(\mu_{1d}\); 15.544 p=5 mix; 15.546 p=7 mix \(109/2863<|T|\); Aut_e \(p\ge11\) / \(A_{\mathrm{full}}\) open; `type_I_multilevel_bad_case_ND_closed=False` |
| \(\lambda_{\min}(\Phi)\ge6\) | **OPEN** | 15.279; 15.549 \(K_\lambda^{\mathrm{all}}\) unnamed; `phi_F_ge_6=False` |
