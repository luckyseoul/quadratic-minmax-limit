...
No leftover flag flipped. Remaining Max+-free estimates isolated in
...
`E_-[S^2]<20+12/p` (or Walsh pair-slice span for interior 4-level only);
leftover 3 is `|μ|≤(p-2)/(2p^2)` on |κ|=1. p=13 orbits are not a close.
Gsum / pairing / Aut-Schur False. L OPEN. Live `e1` is still the old AND.
