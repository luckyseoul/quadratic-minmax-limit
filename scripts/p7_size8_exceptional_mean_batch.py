#!/usr/bin/env python3
"""Parallel exact mean-allocation batches for exceptional p=7 conic orbits."""
...
from p7_fixed_boundary_mean_allocation_batch import run as run_boundary  # noqa: E402
...
def exceptional_orbits(source: dict) -> tuple[tuple[int, tuple[int, ...]], ...]:
    if (
    ...
    if len(out) != 7:
        raise AssertionError(f"expected seven exceptional orbits, found {len(out)}")
    return tuple(out)
...
def valid_existing(path: Path, boundary: tuple[int, ...]) -> dict | None:
    ...
    if (
        ...
        and int(payload.get("allocation_count", -1)) == 180
        and len(payload.get("leaves", [])) == 180
    ):
        return payload
...
def solve_one(
    ...
    path = output_dir / f"cminus_exceptional_orbit{orbit_index:02d}_means.json"
...
def run(
    ...
    return {
        "experiment": "p7_size8_exceptional_mean_batch",
        "status": "complete_exact_exceptional_orbit_mean_batches",
        "p": 7,
        "c_H": -1,
        "source": str(source_path),
        "exceptional_orbit_count": len(rows),
...
def main() -> None:
    ...
    parser.add_argument("--seconds-per-leaf", type=float, default=2.0)
    parser.add_argument("--seed", type=int, default=15708201)
