...
#define ORDER 14
#endif
constexpr int N=ORDER, Q=N-1, XS=1<<(N-1), YS=1<<N, INDEX_BITS=2*N-1;
...
__device__ inline unsigned long long packed(unsigned maskx,unsigned masky) {
  ...
  unsigned long long index=((unsigned long long)masky<<(N-1))|maskx;
  return ((unsigned long long)(unsigned)value<<INDEX_BITS)|index;
}
__global__ void score(unsigned long long* answer) {
  ...
  for(unsigned long long i=(unsigned long long)blockIdx.x*blockDim.x+threadIdx.x;
    ...
    unsigned long long v=packed((unsigned)(i&(XS-1)),(unsigned)(i>>(N-1)));
    if(v>local) local=v;
  }
...
int host_x(unsigned mask,int i) { return i==0 ? 1 : ((mask>>(i-1))&1 ? 1 : -1); }
int main(int argc,char**argv) {
  if(argc!=2){std::cerr<<"usage: hip_scorer B.txt\n";return 2;}
  int8_t C[N*N]={},B[N*N],chi[Q]={};
  for(int a=1;a<Q;++a)chi[(a*a)%Q]=1;
  for(int a=1;a<Q;++a)if(!chi[a])chi[a]=-1;
  for(int j=1;j<N;++j)C[j]=C[j*N]=1;
  for(int i=0;i<Q;++i)for(int j=i+1;j<Q;++j)C[(i+1)*N+j+1]=C[(j+1)*N+i+1]=chi[(j-i)%Q];
  ...
  int source=0;
  for(unsigned mask=0;mask<(unsigned)XS;++mask){int e=0;for(int i=0;i<N;++i)for(int j=i+1;j<N;++j)e+=C[i*N+j]*host_x(mask,i)*host_x(mask,j);source=std::max(source,e<0?-e:e);}
  unsigned x=(unsigned)(result&(XS-1)),y=(unsigned)((result>>(N-1))&(YS-1)); int energy=host_energy(C,B,x,y);
  std::cout<<"{\"n\":"<<2*N<<",\"source_phi\":"<<source<<",\"phi\":"<<(result>>INDEX_BITS)<<",\"energy\":"<<energy<<",\"xmask\":"<<x<<",\"ymask\":"<<y<<",\"workers\":\"hip\"}\n";
}
