#!/usr/bin/env python3
"""Quotient a complete p=7 six-finite floor-survivor recording."""
...
def run(source: Path) -> dict:
    ...
    c_h = int(recording.get("c_H", 0))
    if (
        recording.get("experiment") != "p7_size6_finite_floor_gpu"
        or recording.get("status") != "complete_exact_integer_floor_budget_sieve"
        or int(recording["p"]) != 7
        or c_h not in (-1, 1)
        or int(recording["boundary_size"]) != 6
        or int(recording["infinity_value"]) != 0
        or int(recording["checked_boundaries"]) != 13_983_816
    ):
    ...
    if (
        ...
        or any(len(row) != 6 or tuple(sorted(row)) != row for row in survivors)
    ):
        raise AssertionError("floor-survivor list is not a canonical set of six-sets")
    ...
    for representative in survivor_rows:
        ...
        rows = direction_profile(7, c_h, 0, representative, data)
        profile = tuple(
            sorted(
                (
                    eps,
        ...
        profile_histogram[profile] = profile_histogram.get(profile, 0) + len(orbit)
        orbits.append(
            {
        ...
        remaining -= orbit
    if remaining:
        raise AssertionError("single-pass quotient left unassigned survivors")
    ...
    return {
        ...
        "boundary_size": 6,
        "infinity_value": 0,
        "all_boundaries_in_scope": 13_983_816,
