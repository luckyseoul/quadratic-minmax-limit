...
rational primal/dual solutions. It also corrects a factor-16 normalization
error in the old p=11 norm-20 and norm-24 audit rows of 15.665. The aggregate
trace cone contracts the feasible targets but remains far too wide to prove
R1; the next live refinement is channel-resolved shell moments, while an
all-prime R1 bound still requires a uniform certificate.
...
graph TD
    ...
    R1 -. p=11 exact test .-> R1TRACE["scalar + quartic-trace theta reconstructed to e=800<br/>aggregate conserved-mass cone <b>INSUFFICIENT</b> (15.667)"]
...
### Exact Paley-lattice structure (props 15.629–15.641, 15.665, 15.667)
...
modular forms, and certifies the resulting cone through exponent 800. Seven
of its eight endpoints are already unchanged after exponent 120; the sole
late contraction is still insufficient. These are a substantial structural
advance, but they do not prove R1, global QVAR, E(1), or the limit.
...
| aggregate scalar trace conservation closes p=11 R1 | **falsified exactly** through exponent 800: all eight endpoints have rational primal/dual certificates, seven are unchanged between truncations 120 and 800, and the final component intervals still extend roughly from `-888` to `+875` (15.667); channel-resolved moments are not killed |
...
R1 structure is sharper: Props. 15.629–15.641, 15.665, and 15.667 identify the integral glue,
...
p=11 scalar and trace series through exponent 800. The exact aggregate cone
does not close R1. The missing step is now explicitly channel-resolved shell
information or a stronger modular/multi-scale transport inequality—not an
unidentified glue-class phase, an unknown scalar trace, or any classified low
shell.
...
1. **Spectral floor:** prove global mixed-`k` QVAR and the principal R1
   ...
   At `p=11`, the ordinary and scalar quartic-trace theta forms are now known
   exactly through exponent 800, but their aggregate conserved-mass cone is
   insufficient. The next concrete attack is to recover broad-channel shell
   masses from `tr(R_s O)` and `tr(R_s O²)` (or finer PSL-twisted traces),
   followed by an all-prime transport argument.
...
python3 -m pytest tests/test_prop15628.py tests/test_prop15629.py tests/test_prop15630.py tests/test_prop15631.py tests/test_prop15632.py tests/test_prop15633.py tests/test_prop15634.py tests/test_prop15635.py tests/test_prop15665.py tests/test_prop15667.py -q
