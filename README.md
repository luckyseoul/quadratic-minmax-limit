...
the entire candidate shell `2p||u||^2=2(p+3)` is empty. The principal
...
### Exact Paley-lattice structure (props 15.629–15.638)
...
its first three complete dual shells for `p>=11`, followed by a proved
empty candidate shell. The second shell cancels the first in every channel,
and the complete third shell is another negative channel. These are a
...
R1 structure is sharper: Props. 15.629–15.638 identify the integral glue,
the first three complete dual shells, the radial Poisson phase, and their
exact harmonic operators, and prove that the first possible shell after
them is empty. The missing step is now explicitly a certified tail or
multi-scale theta inequality for the next nonempty and later dual shells,
not an unidentified glue-class phase or any classified low shell.
