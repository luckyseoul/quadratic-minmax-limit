# Quadratic minimax limit

Does the sequence

`alpha_n = n^(-3/2) min_{a_ij in {+1,-1}} max_{x in {+1,-1}^n} |sum_{i<j} a_ij x_i x_j|`

converge as `n` tends to infinity?

The original MO limit is OPEN. A proof of existence need not identify its
value. The current author-reviewed bounds are
`B_infty <= liminf alpha_n <= limsup alpha_n <= 1/2`, where
`B_infty > B_2 > B_opt > B_noise > B_both > B_int > B_tilt = 0.3258530333538241...`.
Here `B_infty` is the supremum of an explicit infinite hierarchy of
fixed-finite-depth ACTUAL successor certificates; every rung is strictly
stronger than the preceding rung. The construction and proof are in
[CORE.md, Section 4](CORE.md#4-a-uniform-gaussian-lower-bound) and
`evidence/NOTE_2026-09-24_TIE_RESOLVED_INFINITE_LADDER.md`. The previously
reported two-step numerical locator is about `0.3258669876`; no new decimal
is assigned to `B_infty`.

Start with [CORE.md](CORE.md) for the route-neutral mathematics,
[STATUS.md](STATUS.md) for current claims, and [HANDOFF.md](HANDOFF.md) for
continuation. [ARTIFACTS.md](ARTIFACTS.md) records the 2026-09-05 reset and
the complete archived research states.

The long [solution.md](solution.md), proof notes under `evidence/`, and
theorem checkers under `src/` retain scoped mathematical results. They are
not a complete solution. Historical Paley, residual-(ii), and amplification
ledgers are optional research routes, not global acceptance criteria.

Global status is tracked in `src/original_mo_status.py`; the old E1 status
interface is retained only with separate route-local diagnostics.

A `ReviewedCompletionProof` entry may optionally pin a machine-checked
formalization (`formal_proof_path`/`formal_proof_sha256`, plus a free-text
`formal_verification_note`) alongside its theorem/review pair, following the
provenance pattern of a human-reviewed writeup plus a hash-pinned, ideally
independently-re-kernel-checked formal proof. This is additive schema only:
it does not change how an entry is admitted, and the production registry
remains empty (`OPEN`) regardless.
