...
2. **Residual (ii), even \(k\ge4p\):** OPEN. The Walsh slice, Eulerian
   ...
   intermediate odd-fibre profile. Proposition 15.675 also closes the first
   all-finite survivor above `3(p-1)/4` for `p=3,5 mod 8`. The other two
   residue classes, the infinity-plus-`p` shell, larger all-finite profiles,
   and full graph constraints remain open.
