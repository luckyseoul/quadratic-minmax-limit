...
1. For every prime \(p\ge5\), E(1) is proved Max+-free on Paley \(n=p^2+1\). The corrected gate is:
   - **Required bi-tight levels 2 and 3:** **satisfied by 15.720.** From 15.272/15.207, a centered bi-tight indicator lies in `scheme+cross`; 15.720 forces all degrees into one residue class modulo \((p^2-1)/2\), and the handshake identity excludes both levels. It also excludes *bi-tight* level 4, but not the one-sided tight level-4 cases inside residual (ii). Proposition 15.167's spectral implication is retracted. Global QVAR, principal R1, and the spectral floor are no longer acceptance units.
