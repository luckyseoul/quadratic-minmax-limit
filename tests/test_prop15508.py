"""Tests for Prop 15.508 — Burnside ω-class; D / Q_τ still unnamed."""
...
def test_fix_is_p_minus_chi():
    B = prove_B()
    assert B["proved"] is True
    assert fix_named(5) == 4
    assert fix_named(7) == 8
    assert fix_named(5) != fix_wrong(5)
    assert fix_named(7) != fix_wrong(7)
    assert fix_wrong(5) == 6
    assert fix_wrong(7) == 6


def test_drop_class_breaks_burnside():
    C = prove_C()
    assert C["proved"] is True
    assert C["rows"]["5"]["after_drop"] != 2
    assert C["rows"]["7"]["after_drop"] != 8
