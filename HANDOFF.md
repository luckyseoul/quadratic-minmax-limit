...
> **Now (2026-08-17 evening):** Leftover 1 still **OPEN**. 15.453–15.486 shipped. The 15.276 family is TR-only (\(c_{\mathrm{family}}=4\neq19\) at p=7); it does not pin \(c=c_{\mathrm{eq}}\). \(Q_\tau\) unnamed. p=11 6-net — leave PID 712954. Do **not** import \(\phi_F\). Residual (ii) \(k\ge4p\) and Type I multi-level still OPEN. Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
