...
> **Now (2026-08-17 evening):** Leftover 1 still **OPEN**. 15.453–15.477 shipped. Triple-rainbow \(\Delta^2\) named: \(p S_\chi^2\) if \(p\equiv1(4)\), \((M/\lambda)^2\) if \(p\equiv3(4)\). Other complementary types and \(Q_\tau\) unnamed. p=11 6-net — leave PID 712954. Jellyfin: 16-core + Arc oneAPI. Orin: TRT/CUDA spare. Do **not** import \(\phi_F\). Residual (ii) \(k\ge4p\) and Type I multi-level still OPEN. Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
