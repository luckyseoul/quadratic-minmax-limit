...
> **Now (2026-08-17 evening):** Leftover 1 still **OPEN**. 15.453–15.481 shipped. \(W_c\) is the E-sum on \(y^2=x^3-x\); named \(K_{++}\) recovers live \(Q_{++}\). \(Q_\tau\) is not a rational in \(p\). p=11 6-net — leave PID 712954. Do **not** import \(\phi_F\). Residual (ii) \(k\ge4p\) and Type I multi-level still OPEN. Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
