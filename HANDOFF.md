...
> **Now (2026-08-18):** **Three of four leftovers remain.** Lemma D is already True (do not unflip). Still OPEN: (1) floor \(\lambda_{\min}(\Phi)\ge6\) / name \(Q_\tau\) (15.504: two-level \(C\) of \(u\) on \(\mathbb{F}_p^\times\)-orbits is tautological at p=5,7 by evenness and \(|(p-1)/2|\in\{2,3\}\), and fails for Type+ 1D at p=13. Does not name \(Q_\tau\)); (2) residual (ii) even \(k\ge4p\); (3) Type I multi-level. Do **not** import \(\phi_F\). Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
