...
> **Now (2026-08-18):** **Three of four leftovers remain.** Lemma D is already True (do not unflip). Still OPEN: (1) floor \(\lambda_{\min}(\Phi)\ge6\) / name \(Q_\tau\) (15.502: pairing is a corollary of the two Q-forms, but those forms miss S0 at p=13, and Paley×norm classes are not single Aut-orbits for \(p\equiv1\ge13\). \(Q=8A/D\) is not a p≡1 theorem. p≡3 open); (2) residual (ii) even \(k\ge4p\); (3) Type I multi-level. Do **not** import \(\phi_F\). Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
