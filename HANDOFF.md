...
- **Prop 15.55 (tight size-\(2p\) obstruction):** If \(\lambda_{\max}(G)=n/2\) simple, then **no** Max+-tight level-2 cover of size \(2p\) (bi-tight empty; Type I freeness-failure blocked). **Proved** conditional; **certified** hypothesis at \(p=5,7\); fails at \(p=3\) (\(\lambda_{\max}=8>5\)). Evidence: `e1_gmin_tight_obstruction.json`. **OPEN:** prove \(\lambda_{\max}(G)=n/2\) for all primes \(p\ge5\).
- **Residual to \(m_n\ge\Phi-2\Rightarrow L=\tfrac12\):** (1) \(\lambda_{\max}(G)=n/2\) for all \(p\ge5\) **or** \(g_{\min}>T(p)\); (2) deep non-tight ND. **F13/F16–F18** intact.
