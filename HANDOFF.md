...
> **Now (2026-08-18):** **Three of four leftovers remain.** Lemma D is already True (do not unflip). Still OPEN: (1) floor \(\lambda_{\min}(\Phi)\ge6\) / name \(Q_\tau\) (15.501: on the whole Hoffman window \(D\ge D_{\mathrm{need}}\) for every \(p\equiv1\), so pinning \(c=c_{\mathrm{eq}}\) is not required for the p≡1 pairing bound; still need \(Q=8A/D\) and 15.497 pairing as general identities. \(D\) unnamed. p≡3 open); (2) residual (ii) even \(k\ge4p\); (3) Type I multi-level. Do **not** import \(\phi_F\). Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
