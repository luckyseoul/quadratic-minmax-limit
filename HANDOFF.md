...
> **Now (2026-08-18):** HEAD 15.554. **Three leftovers remain.** Floor: 15.554 \(K_{\mathrm{all}}=\mathrm{Term0}+\mathrm{Cross}+\Omega\Omega\); \(\hat y\) \(k\)-point supported on \(\sum\alpha_i=0\); Q3 constant on 3-line / double / generic 1-line (live values, not a \(p\)-law); Q4\(\neq\)Wick. 15.553 Term0 named. 15.550 \(S=\mathrm{Kl}\). Ω-bulk values still unnamed. 15.549 \(K_\lambda=K_{\mathrm{all}}-Nq(3q-5)\). Residual (ii): 15.552 leftover+splus empty nF=9,11–13 at p=5 \(k=22\) (nF=10 TLE). Type I: 15.551 Galois line-union. \(\phi_F\) not imported. \(L=\tfrac12\) is **not settled**.  
