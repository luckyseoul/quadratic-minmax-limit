...
> **Now (2026-08-17 evening):** Leftover 1 still **OPEN**. 15.453–15.475 shipped. Parallel-union class \(\Delta^2\) named (\(M^2\) if \(p\equiv3(4)\), \(p^5(p-1)^2/4\) if \(p\equiv1(4)\)). Complementary \(\Delta^2\) and \(Q_\tau\) unnamed. p=11 6-net — leave PID 712954. Jellyfin (16-core Ryzen, Arc A380) is extra CPU when soulkiller is pegged. Do **not** import \(\phi_F\). Residual (ii) \(k\ge4p\) and Type I multi-level still OPEN. Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
