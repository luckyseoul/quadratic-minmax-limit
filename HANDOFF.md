...
> **Now (2026-08-17 evening):** Leftover 1 still **OPEN**. 15.453–15.482 shipped. \(W_c\) is real \(\Phi(\mathrm{Tr},N)\); the 15.481 pairing against \(E[N]\) is identically 4, not live \(Q_{++}\). Leftover is the \(N_\perp\) / 4-point. 48 is not a Gaussian norm. \(Q_\tau\) unnamed in \(p\). p=11 6-net — leave PID 712954. Do **not** import \(\phi_F\). Residual (ii) \(k\ge4p\) and Type I multi-level still OPEN. Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
