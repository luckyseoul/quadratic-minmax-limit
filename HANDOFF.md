...
> **Now (2026-08-18):** HEAD 15.522. **Three leftovers remain.** Floor: live \(Q_\tau\cdot D\) is a \(\mathbb Z[\sqrt{-2}]\) norm at p=5,7 (15.522). Fail: same in \(\mathbb Z[i]\) (48 not \(a^2+b^2\)). \((a,b)\) not a p-law (`10p-46` interpolates \(a_{++}\) at \{5,7\}). Residual (ii): leftover+s₊=2 at p=5 \(k=20\) empty for nF=0..6 (15.521). nF≥7 and even \(k>4p\) stay open. \(\phi_F\) not imported. \(L=\tfrac12\) is **not settled**.  
