...
## 2026-09-13: matched-block gluing — exact minima and a corrected reading of the cross floor

Session started as a duplication check (user-requested) on gluing experiments
and ended with a small new finite result plus one convention correction.

**Duplication audit.** The gluing mechanism duplicates the recorded
completion route (`NOTE_2026-09-12_COMPLETION_DISCREPANCY_FRAMEWORK.md`
prior-art notice; `NOTE_2026-09-11_ORDER16_HUNT.md`; the block identity is
`NOTE_2026-09-01_ORIGINAL_LIMIT_TWO_RAY.md` eq. (4)). The sub-family here
differs: both diagonal blocks are chosen (matched optimal blocks of orders
n and m), not fixed to one recorded source.

**New finite result.** With `phi_A = m_n`, `phi_C = m_m` and a free cross
block, full enumeration of all completions gives exact minima
**10 (4+4, 2^16), 12 (4+5, 2^20), 13 (5+5, 2^25)** — each equal to the
recorded `m_8, m_9, m_10`. Optimal-completion counts: 184, 10800, 1840.
So at N=8,9,10 the matched-block gluing family attains the merged order's
optimum, meeting the doubling threshold with equality.

**Corrected reading of the recorded cross floor.** The recorded
`max_{x,y}|x^T B y| >= (sqrt(2/pi)-o(1)) n^{3/2}` floor maximizes over the
cross block; the gluing object minimizes over it. By the both-sided bound
`phi_A + phi_C - nm <= Phi(merged) <= phi_A + phi_C + nm` (proved in the
note), the floor does not lower-bound the gluing minimax, and the doubling
question is a joint alignment/negativity statement about
`(A, C, eps)`, not a cross-term margin. This corrects the reading of the
floor used in the September 12 completion writeups; the recorded
statements themselves are not retracted, only their scope is.

**Honesty record.** An intermediate kernel evaluated split sums on a
reduced half-cube, which is invalid for directed split sums; one
verification fragment reported 19 for a 5+5 witness whose true value is 13
(literal loop and full-cube kernel agree). The kernel and all earlier
JSONs were removed; `scripts/glue_exact_raw.py` (full-cube, validated
against `m_2..m_6 = 1,3,4,4,5`) and `raw_glue_*.json` are the surviving
artifacts.
