...
> **Now (2026-08-17 evening):** Leftover 1 still **OPEN**. 15.453–15.484 shipped. TR \(L_{++}\) is not a function of \(p\) (splits on constructed p=11 Max+). The 2-point interpolant \(\mu_+^2-\chi+(1-\chi)/12\) dies at p=11. \(Q_\tau\) unnamed in \(p\). p=11 6-net — leave PID 712954. Do **not** import \(\phi_F\). Residual (ii) \(k\ge4p\) and Type I multi-level still OPEN. Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
