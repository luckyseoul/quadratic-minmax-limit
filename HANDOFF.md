...
- **Prop 15.60 (antipodal → projective ENTF; 2026-07-30):** **Proved:** \(T(x)=T(s)\) for antipode-symmetric \(s\); gap \(\Leftrightarrow\lambda_2(W^{(\mathrm{proj})})\le m/(2d)\) on Max+/± of size \(m=N/2\); projective set is ENTF; **2×spherical fourth-moment \(\Rightarrow\) gap for all \(d\ge6\) (i.e. \(p\ge5\))**. **Certified:** 2×sphere holds \(p=5,7\) (ratios \(\sim1.95,1.43\)), fails \(p=3\) (\(\sim2.8\)). Evidence: `e1_gmin_projective.json`. **OPEN residual:** prove fourth-moment Rayleigh \(\le 4m/(d(d+2))\) on projective Max+ for all primes \(p\ge5\). Bi-tight **not** yet closed for general \(p\).
- **Residual to \(m_n\ge\Phi-2\Rightarrow L=\tfrac12\):** (1) 2×sphere / Veronese for all \(p\ge5\) (closes bi-tight via Prop 15.55); (2) deep non-tight ND. **F13/F16–F18** intact.
