...
**Status date:** 2026-07-30 (Prop 15.65 κ spectrum + λ2(P⊙P)≤4/N form; L still OPEN)  
...
- **Prop 15.65 (κ spectrum; clean \(\lambda_2(P\odot P)\le4/N\); boolean essential; 2026-07-30):** **Proved:** \(16N\Leftrightarrow\lambda_2(P\odot P)\le4/N\Leftrightarrow\lambda_2(W)\le4N/d^2\); \(P\odot P=\alpha^2 W\). **Certified:** κ|Z eigs \(\{8\}\) at p=3; \(\{-24,40,72\}/13\) at p=5 (top=budget); p=7 top \(1048/409\). **Certified counterexample:** random equal-diag projectors violate \(4/N\) (e.g. \(N=50,d=10\)), so boolean/conference is load-bearing. Evidence: `e1_gmin_cumulant.json`. **OPEN residual:** prove \(\lambda_2(P\odot P)\le4/N\) (or H) for Max+ of all primes \(p\ge5\). Bi-tight **not** closed for general \(p\).
- **Residual to \(m_n\ge\Phi-2\Rightarrow L=\tfrac12\):** (1) prove \(\lambda_2(P\odot P)\le4/N\) or H or \(g_{\min}\ge L(p)\) for all \(p\ge5\) (any closes bi-tight); (2) deep non-tight ND. **F13/F16–F18** intact.
