...
> **Now (2026-08-17 evening):** Leftover 1 still **OPEN**. 15.453–15.476 shipped. \(\Delta^2\) is a function of AG(2,p) direction type (0 split). Parallel-union \(\Delta^2\) named (15.475). Complementary values unnamed. p=11 6-net — leave PID 712954. Jellyfin: 16-core + Arc A380 oneAPI/SYCL. Orin: TRT/CUDA spare, torch CPU-only. Do **not** import \(\phi_F\). Residual (ii) \(k\ge4p\) and Type I multi-level still OPEN. Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
