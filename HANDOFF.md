...
**Status date:** 2026-07-30 (Prop 15.64 dual Φ form of H; L still OPEN)  
...
- **Prop 15.64 (dual \(\Phi\) form; residual reformulation; 2026-07-30):** **Proved:** \(\max Q=N\lambda_{\max}(\Phi|_{\mathcal Z})\) on zero-diag \(\cap V_+\); \(K=(D\odot D)/(2N)\) on \(\mathbf1^\perp\); Wick baseline 8; H \(\Leftrightarrow\) residual\(\le(p+1)(p+7)/d\). **Certified** exact gen.eig p=3,5,7 (`e1_gmin_H_proof.json`); K spectrum p=5 \(\{13,88/13,72/13,40/13\}\). **OPEN residual:** prove \(\lambda_{\max}(\Phi|_{\mathcal Z})\le6+2H(p)\) for all primes \(p\ge5\). Bi-tight **not** yet closed for general \(p\).
- **Residual to \(m_n\ge\Phi-2\Rightarrow L=\tfrac12\):** (1) prove H / residual bound / 16N for all \(p\ge5\) (closes bi-tight); (2) deep non-tight ND. **F13/F16–F18** intact.
