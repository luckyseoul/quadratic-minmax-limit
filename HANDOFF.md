...
> **Now (2026-08-18):** **Three of four leftovers remain.** Lemma D is already True (do not unflip). Still OPEN: (1) floor \(\lambda_{\min}(\Phi)\ge6\) / name \(Q_\tau\) (15.503: every Max+ at p=7 has \(Q_y\) constant on \(\mathbb{F}_p^\times\setminus\{\pm1\}\) though Aut only forces inv-pairs; not constant per design on coarse ++. Ensemble ++ still flat. No general reason yet); (2) residual (ii) even \(k\ge4p\); (3) Type I multi-level. Do **not** import \(\phi_F\). Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
