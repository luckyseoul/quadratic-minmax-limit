...
`7044d8263f2fd6556549a3917d2a53f7fcf2ace2`. The existing uncommitted
15.773 work and the protected 15.766/15.767 records are preserved.

Proposition 15.774 closes `p>=29,t=q+1,q+2,k=5p+1,5p+3`, where
`q=(p-1)/2`, and proves separate minimal-four-gap support bounds.
The new local theorem says `0<4p E[C]<2p-10` forces a nonnegative
integral quadratic on `J(p,(p+1)/2)` to be Boolean of mass `p-3` or
`p+1`. The affine
even-boundary parity union is `{0,p-3,p-1,p+1}`; the endpoint `2p-10`
is deliberately not classified. Use both type residues and the necessary
capacity `t>=u+m*k_min(u)`, with `m=(p+1)/2`, not merely one row mean.
For `r=3,4,5` the signed shell exclusions reach `5p+4,6p+4,7p+6` at
`p>=37`, or `5p-12,6p-12,7p-12` for every `p>=29`.

The two residual carries cover the missing p29/p31 endpoints as well.
If quotient one is absent at `u<q`, retain the new quotient-two branches
`u<s`; at `u=q`, retain both zero and no-zero alternatives. Use actual
low `P<=9`, with `P+Q>=9` at `P=9`, and recompute the hard phase
after transport. No mean-`2p` equality catalog or prime census is needed.

The next unclaimed generic layer is `t=q+3,k=5p+5`.
Thus the live generic frontier is `t>=q+3,k>=5p+5`.
The unchanged small-prime frontiers are critical `p=5,7`;
`p=11,t>=3` (`k>=50`); `p=13,17,19,t>=5`; `p=23,t>=12`
(`k>=116`); and positive `p=7,z=7`.
Odd minimal four-gap sets require `|H|>=5p+6`. Even and odd-no-bridge
sets satisfy the corresponding r4/r5 lower bounds, but no theorem
localizes every minimal witness into these bands. At the first uncovered
eventual layer `t=p+1+ceil(r/2)`, exact scalar quota survivors already
exist. They are not graph realizations. The remaining work needs more
than the same scalar spectrum and quotas.

Authoritative proofs are
`evidence/NOTE_2026-09-04_SHARP_SMALL_MASS_SPECTRUM.md` and
`evidence/NOTE_2026-09-04_SMALL_MASS_TWO_TYPE_BRIDGE.md`; source is
`src/e1_gmin_m4_small_mass_spectrum.py` and `src/e1_gmin_m4_prop15774.py`.
...
The 42-file technical suite exited zero in 34.19 seconds from
`soulkiller:/tmp/small-mass-two-type.CoBLYz/regression`, with JUnit at
`/tmp/small-mass-two-type.CoBLYz/technical.junit.xml`. The provenance
record is `evidence/small_mass_two_type_regression.json`; no controller
tests or solvers were run. Carried dependencies may replay the existing
fixed four-bit exact certificate; the new helper enumerates no catalog.
No new prime, graph, or local equality census was launched.

Replay the complete pinned manifest from the checkout, or from
`soulkiller:/tmp/small-mass-two-type.CoBLYz/regression`:

```bash
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src:. /home/nick/.venvs/mo-exact/bin/python -m pytest -o addopts= -q -n 0 $(jq -r '.test_files[]' evidence/small_mass_two_type_regression.json)
```

The provenance record holds the frozen proof hashes and the separately
measured technical and final documentation results.

Eventual E1 at all sufficiently large primes would suffice for `L=1/2`
via denseness, but bounded-size exclusions are not eventual E1.
Global residual (ii), the all-size implication bridge, E1, and the
original MO limit remain OPEN.

## Historical completed verified target: Proposition 15.773 (2026-09-04)

This preceding checkpoint and its measured validation are retained
unchanged in scope. Its then-next two generic layers are closed by
15.774 above; its frontier statements are historical.

The active checkout is `/home/nick/quadratic-minmax-limit-residual-ii`, branch
`residual/p13-u6-common-moments`; this attack began at clean checkpoint
`7044d8263f2fd6556549a3917d2a53f7fcf2ace2`. This is not the separate
`main` worktree.

Proposition 15.773 closes `p>=29,t=q,k=5p-1`, with `q=(p-1)/2`, for
every boundary size in both congruence classes. The hard quotient sum at
`|H|=5p` is `p-u`. Every `u<q` residue has at least `u+1` low rows,
and the 15.769--15.772 classifications and local exclusions still apply.
In the p3 sharp branch, the two excess quotient units may lie in one
quotient-three row or two quotient-two rows; both give `hard edges=mP+2`
and force seven mass-`p+13` opposite rows. Do not assume a unique high row.

The genuinely new `u=q` all-one branch needs no equality classification.
The common actual parallel count has `0<=P<=9`,
`hT=(p+1)P-5p`, and `a_opp(Q)=(p+1)(P+Q)-8p`. Negative means and
forbidden mass eight give `P+Q>=9`; opposite surplus `m-5` forces five
mass-`p+9` rows, already excluded by 15.752 and pointwise phase-zero
baselines. At `P=9`, the mass-eight index would be `Q=-1`; no actual
row at that index is assumed. The quotient-zero endpoints remain separate
and force the same five mass-`p+9` rows.

Authoritative proof:
`evidence/NOTE_2026-09-04_JOINT_5P_MINUS_ONE_CLOSE.md`.
Exact theorem package: `src/e1_gmin_m4_prop15773.py` and
`tests/test_prop15773.py`. Complementary mesh arithmetic is recorded in
...
Testing and computational verification for this target are offloaded;
no test or solver suite is run on the controller. The staged regression
worktree is `soulkiller:/tmp/joint-5p-minus-one.XZVoSh/regression`.
All four mesh modes passed: soulkiller checked exact bivariate identities
for arbitrary actual `P`; NUKA checked the p1 carry ledger; jellyfin
checked the p3 sharp, quotient-zero, and excluded mass-`p-1` branches;
Orin checked rational parity floors and ARM64 ledger portability, including
the `P=9` legal-index case. The five checker tests passed on soulkiller
in 0.38 seconds.

SHA-256 values:

- `src/e1_gmin_m4_prop15773.py`:
  `4a14b742600a32c9535fa7d720a4180a10240f1739063a625895d9d77babf910`.
- `tests/test_prop15773.py`:
  `04001da3e35f5c1dfaf5fae0e49675037a34444f480b0adcbbc60feb40d4c70e`.
- `evidence/e1_gmin_m4_prop15773.json`:
  `7b77b9138ac2ad474857310599c5f440e235f3f07dbf45680463305b8452a266`.
- `evidence/NOTE_2026-09-04_JOINT_5P_MINUS_ONE_CLOSE.md`:
  `3d21cfc5ef0c4533d2628986b5f5f4203fa873922c554235f3426cf592f1bfd5`.

- `scripts/joint_5p_minus_one_mesh_check.py`:
  `a1569a12d021f1ab5ab8cc9d521f5777aef08f6c32c7a481c2fe0eeb10f5744f`.
- `evidence/joint_5p_minus_one_mesh_replay.json`:
  `bbbdb1d5aab9c50a8e7a3f8e349147e32ba488b1894142d112a539183484105b`.

The final 39-file offloaded regression passed **385 tests in 33.55 seconds**
on soulkiller, exit zero. It covers the 28 test files from `f8af445`,
the later endpoint and mesh packages, the four local dependency suites,
the new 15.773 tests, and the documentation gate. This includes the
additional false-dependency injection tests: the theorem wrappers fail
closed if any row identity, residue partition, local exclusion, or branch
certificate loses its proof flag. The saved/live theorem JSON agrees.
The proof, code, and complete carry table passed independent adversarial
review. The mesh checks are not a new prime, graph, or local equality census.

The exact sorted test manifest, remote command, frozen file hashes,
captured output, and JUnit hash are recorded in
`evidence/joint_5p_minus_one_regression.json`. Replay from the checkout:

```bash
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src:. /home/nick/.venvs/mo-exact/bin/python -m pytest -o addopts= -q -n 0 $(jq -r '.test_files[]' evidence/joint_5p_minus_one_regression.json)
```

All source and theorem-evidence files are unchanged after that frozen
regression. This measured-result handoff update is documentation only;
its final documentation gate is replayed separately on soulkiller.
This target's changes are uncommitted and unpushed; the starting commit
remains `7044d8263f2fd6556549a3917d2a53f7fcf2ace2`.

The chosen mathematical target is complete. The next unclaimed generic
layer is `t=q+1,k=5p+1`; it is not attacked in this checkpoint.
Do not reopen `k=5p-1` with a mean-`2p` equality classification.
The direct original-MO route remains separate and open.

## Historical completed full-mesh target: Proposition 15.772 (2026-09-04)

This section preserves the preceding p1 checkpoint, including its replay
and hashes. Its then-next generic layer was subsequently completed by
15.773 above; the frontier recorded here is historical.
...
attack. The active checkout is `/home/nick/quadratic-minmax-limit-residual-ii`
on that branch, not the separate `main` worktree.

Proposition 15.771 now **closes `p=23,t=11,k=114` for every boundary size**.
The general-slice `b=4,20` equality proof, the covering swap-cube proof for
`b=6,8,...,18`, and the phase-zero mass-32 bridge are explicit and have
passed independent proof review. The new hard equality offsets are exactly
`4,5,6,7,8`; both quotient-profile branches force at least five opposite
rows of mass32, and every such boundary is excluded. The p23 frontier
therefore starts at `t=12,k=116`. No generic frontier or global gate changes.

The sole chosen target is completed; no later-layer search is running.
Do not extend the old five-set certificate or rerun a graph/slice census at
`t=9,10,11`. The next deliberately selected Paley attack should reassess
the new all-mean-46 equality theorem against the still-open generic third
post-band layer (`p=1 mod4,p>=29,t=q-1`), before launching any computation.
That implication has not been proved or attacked in this checkpoint.
The direct original-MO route remains separate and open.

Authoritative proof: `evidence/NOTE_2026-09-04_P23_THIRD_POST_BAND_CLOSE.md`.
Complementary mesh checks all exited zero: soulkiller checked nine rational
contact quadratures and phase-one LP floors; NUKA certified both small-side
contact ranks250 with27 independent kernel witnesses; jellyfin checked all
seven covering cubes and exact Walsh Gram identities; Orin checked the
phase-zero floors and row ledgers with ARM64 exact arithmetic. These are
bounded independent checks, not substitutes for the analytic proof.
No GPU research job, installation, or mount change was needed.

Replay:
`PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src:. /home/nick/.venvs/mo-exact/bin/python -m pytest -q -n 0 tests/test_prop15768.py tests/test_prop15769.py tests/test_prop15770.py tests/test_prop15771.py tests/test_p23_post_band_moment_close.py tests/test_p23_second_post_band_moment_close.py tests/test_p23_third_post_band_mesh_check.py tests/test_main_chain_docs.py`.

Final regression: **238 tests passed**, covering all 28 test files added in
`f8af445`, the new independent mesh-check file, and the documentation gate.
This includes exact equality of the live proposition payload and saved JSON,
proof-note presence, current p23 frontier checks, and all four recorded node
outputs with the pinned checker hash. `git diff --check` is clean.

SHA-256 values:

- `evidence/e1_gmin_m4_prop15771.json`:
  `20ffd08d29bf08f33c9a50d12ea07e7c3c021bb3d6dfe22d0298ee5e7e2de3e2`.
- `evidence/p23_third_post_band_mesh_replay.json`:
  `2cd9c8388bab4f3e320d2f3ca0d296307c3f568b5189cf5fdc978e3a9197b50a`.
- `scripts/p23_third_post_band_mesh_check.py`:
  `72915dde0bec34aaf60ced93bd682b67eff39577e70c03ff7e7f2638cc3b6de4`.

## Historical checkpoint before the full-mesh attack (2026-09-04)

The following pending-review record describes `f8af445`, not the current
verdict; the completed section above supersedes it.

The working branch is `residual/p13-u6-common-moments`, based on
`54932032984c66061977a43f54b80ee41b7cb3f3`. This checkpoint preserves the
post-15.761 work and the reviewed post-band closes through 15.770.
Proposition 15.771 was then an **open reduction / REVIEW_PENDING candidate**
for `p=23,t=11,k=114`, not reviewed endpoint closure. Its executable checks
passed; the public closure flags remained false until the three proof bridges
in `evidence/NOTE_2026-09-04_P23_THIRD_POST_BAND_REVIEW.md` were completed.
The canonical p23 frontier then remained `t>=11`; all global gates were open.

The pre-checkpoint focused run covered all 28 newly added test files plus
`tests/test_main_chain_docs.py` (230 tests passed). The status correction is
replayed with `PYTHONPATH=src:. /home/nick/.venvs/mo-exact/bin/python -m pytest
-q tests/test_prop15771.py tests/test_main_chain_docs.py`. The next single
target is to settle the three explicit proof bridges for this candidate,
using independent exact checks across the mesh, not another graph census.
The status-correction replay passed 22 tests, including the documentation
gate; all newly added source and script files also compile. The pending
15.771 evidence SHA-256 is
`297ccf52d1d3ec7408be79d2fe66e6f243fd4fe4e5cdf61d52a07643258b547f`.
...
- The exact two-half variables `u=(x+y)/2`, `v=(x-y)/2` reduce doubling to
  ...
  `R` for the stored optimal representatives gives `(B,g)=(16,8),(18,8),
  (22,4),(28,8)` at `n=5,6,7,8`.  The sharp zero-error line fails at `5,6`
  and passes at `7,8`; this is finite evidence only.
- Proposition 6.5a removes the geometric disguise completely:
  `K(A,R)=max_U Phi(A^(F_S(U)))`, where `S=A circ R` is a tournament and
  `F_S(U)` is the set of cut edges directed from `U` to its complement.
  Thus choosing the skew half is exactly choosing an orientation whose every
  directed half-cut flip is a `sqrt(2)`-stable neighbor of optimal `A`.
  Global minimality gives only the reverse inequality.  The ordered special
  case is governed by prefix total variation, so prefix switching is
  tautological rather than a proof.
- The `sqrt(2)` factor is critical, not a loose plotting constant.  If the
  optimal directed-half-cut quantity were at most `c m_n+o(n^(3/2))` with
  any fixed `c<sqrt(2)` on a complete eventual dyadic tail, the lift would
  force `alpha_(2n)<=(c/sqrt(2))alpha_n+o(1)` and hence `alpha->0`, contrary
  to the uniform `1/pi` lower bound (Proposition 6.5b).
- A second exact lift came directly from complexifying the plotted diamond:
  `K(A,C)=[[A,C],[C^T,-A]]` has norm
  `max_(x,y)(|Q_A(x)-Q_A(y)|+|x^T C y|)`.  For symmetric
  `C=C_0+D` this is, within additive `n`, twice the maximum norm of an exact
  family of hybrid slices between `A` and `C_0` (Proposition 6.5c).
  Holomorphicity forces the coherent choice `C_0=A`; that specialization is
  a QPSK/clique-flip norm and already fails the tempting zero-loss bound at
  the global order-four optimizer (`L_cl=6>4sqrt(2)`). Proposition 6.5n
  strengthens this to an infinite complete-signing family with
  `Phi(A)=Theta(n^(3/2))` and a coherent gap exceeding
  `1.1657n^(3/2)` asymptotically. This kills every argument using only the
  optimal-order scale, but not one using the exact global-minimizer property
  `Phi(A)=m_n`. General noncoherent,
  `A`-dependent cross blocks remain live.  Their exact four-label expansion
  has a particularly clean all-directed specialization:
  `4D_to(A,S)=max_(x,y)(|Q_A(x)-Q_A(y)|+|x^T R y|)` for
  `R=A circ S`.  Hence orienting every signed outgoing half-cut below
  `m_n/sqrt(2)+o_Dini(n^(3/2))` would close doubling.  Each face separately
  is balanceable below `ceil(m_n/2)`; the missing theorem is simultaneous
  balance across every face by one orientation.
- The envelope defect is now decomposed exactly as
  `D=|Q_A(x)-Q_A(y)|+2epsilon(x,y)`.  A pair is dangerous only when its two
  directed half-cut energies have opposite signs.  Equivalently, the largest
  skew witnesses must avoid simultaneous same-sign near-maximizer layers;
  at the known upper scale, a same-sign skew maximizer needs at least
  `(0.383670998...-o(1))n^(3/2)` combined energy defect.
- Two immediate variational continuations are now closed as methods.
  Single-arc local descent may be blocked solely because tied active cuts
  have no common crossing edge, and the fractional minimax has exact optimum
  `Phi(A)` at the nonintegral point `R=0`.  A targeted arXiv/GitHub/Zenodo
  audit found no directed-cut, switching, or generic discrepancy theorem
  with the required quantifiers and sharp constant.  The missing statement
  is a genuinely global integral completion/rounding theorem.
- Proposition 6.5d makes that statement an exact zero--one covering LP on
  decomposable Boolean bivectors.  The ordinary linear relaxation has value
  zero; the displayed covariance/SDP relaxation is strictly subcritical
  from `n=16`; and normalized single-row even-moment certificates are blind
  in the proved range `p<0.0347679...(n-1)`.  Proposition 6.5j further
  proves that the full degree-four preordering of the exact squared row
  inequalities is blind from `n=45`, even with affine SOS multipliers,
  every pairwise cross-row product, and the instantiated Pluecker identities.
  Proposition 6.5l extends that obstruction to the full raw degree-`2D`
  preordering whenever
  `(2D+1)^D[(1+14.3810675004.../(n-1))^D-1]<1`.  Thus every
  fixed raw degree, and half-degree through
  `(1-o(1))log(n)/log(log(n))`, is eventually blind.  Do not rerun those
  exact relaxations at another fixed raw degree.  Quotient-degree or
  differently lifted encodings, substantially growing raw degree, an
  `A`-dependent nonuniform functional, and direct nonlinear rounding remain
  possible.
- Proposition 6.5e proves a signed-regular Gaussian arcsine bound with a
  positive square correction.  On the sparse block
  `K_0=[[A,R],[-R,-A]]` it yields
  `D_to>=n(n-1)/pi asin(1/sqrt(2n-2))+Sigma/(16pi(2n-2)^(5/2)(1-1/(2n-2))^(3/2))`,
  with `Sigma=sum_(i!=j)((A^2-R^2)_ij^2+(AR-RA)_ij^2)`.  Its leading
  constant `1/(sqrt(2)pi)` exactly meets `m_n/sqrt(2)` at the universal
  `1/pi` floor, and an independent dyadic argument forces outgoing-half
  limsup ratio at least `1/sqrt(2)`.  Near `alpha_n->1/pi`, a successful
  orientation must have `Sigma=o(n^4)`; with only the present upper bound
  that rigidity can be vacuous.  This is analytic sharpness and a structural
  gate, not an upper construction.
- Proposition 6.5f constructs one tournament shielding any prescribed
  anchor family: if its coordinate patterns form `L` nonempty signature
  cells, then `||Rx||_1<=Ln` for every anchor.  Up to
  `L=(0.2636965...+o(1))sqrt(n)` is absorbed by the exact critical margin.
  Together with a stated high-difference vertex-cover condition this would
  close the outgoing-half target.  The construction does not bound
  non-anchor pairs and may have global skew norm `Theta(n^2)`, so it is not a
  close.  It removes only anchor-incident constraints, conditional on the
  still-open vertex-cover condition; the non-anchor layer remains live.
- Proposition 6.5k replaces signature counting by weighted Banaszczyk
  balancing.  If `sum q_a<=1/2` and
  `rho_a=n sqrt(2/pi)+sqrt(2n log(1/q_a))`, the capacity condition
  `50 sum rho_a^2/B_a^2<=1` produces one integral skew signing with
  `||R x^(a)||_1<=B_a` simultaneously.  At the exact outgoing-half budgets
  it shields an arbitrary `k<=(c_*-delta)n` anchors for every fixed
  `0<delta<c_*`, with
  `c_*=(3-2sqrt(2))/(25pi)=0.0021845336957706...`.  This is a
  proved linear-capacity advance, not closure: the dangerous graph and
  `Gamma(R)` belong to the resulting orientation.  One must prove a
  capacity-valid cover for that same `R`; selecting a cover and rerunning
  the rounding is circular.
- Proposition 6.5g proves the approximate square/commutator mate demanded
  near `alpha=1/pi` actually exists: uniform random orientation has exact
  expected `Sigma=sum offdiag(A^2)^2+3n(n-1)(n-2)`, and the arcsine
  correction pays the codegree term from `Phi(A)-L_n`.  Conditional
  expectation derandomizes it.  But the strongest generic spectral bridge
  loses `pi/2` even at zero full defect and can meet the target only at
  `alpha=1/2` with vanishing defect.  Repeating that generic
  trace/Frobenius conversion cannot help; a square/commutator route would
  need additional statewise or special spectral structure.
- Proposition 6.5m rules out the exact commuting-conference specialization.
  For any symmetric signing `A` and skew signing `R` of even order, every
  diagonal entry of `AR-RA` is `2 mod 4`, hence
  `||AR-RA||_F^2>=4n`.  This excludes exact commutation even when `R` is not
  conference.  It does not exclude Proposition 6.5e's approximate
  `Sigma=o(n^4)` mate because its lower bound is only `O(n)`.  Beware the
  terminology reversal: an orthogonal-design amicable symmetric/skew pair
  anticommutes; the desired commuting pair is anti-amicable.
- Proposition 6.5h writes the exact independent-random outgoing-half
  first-moment criterion and proves that its event-orbit sum grows
  exponentially for optimal signings.  This rules out that union-bound
  proof, not random orientations themselves.
- Proposition 6.5i proves the exact Gaussian saturation gap and a
  Hermite-rank-two Hamming-overlap variance bound.  Near `alpha=1/pi`, its
  positive and negative outputs are near-extremizers at Hamming distance
  `n/2+o(n)`.  If a sharp outgoing orientation exists there, applying the
  same theorem to `K_0` forces the central two-half saddle seen in the plot:
  its cross coefficient is `Phi(K_0)-o(n^(3/2))` and each axial coefficient
  is at most `Phi(K_0)/2+o(n^(3/2))`.  This necessary structure proves the
  central layer cannot simply be eliminated; it does not supply the open
  orientation.  The proof is in
  `evidence/NOTE_2026-09-02_GAUSSIAN_SATURATION_CENTRAL_SADDLE.md`.
- Random vertex orders have now been audited without an order census.  For a
  fixed cut and state their signed cross term has exact variance
  `(h+D_A(U,y))/3`; `Phi(A)=M` gives only `D_A<=nM`, permitting variance
  `Theta(n^(5/2))`.  Averaging over states gives the same second moment as
  independent orientations.  An ordered proof would need new
  global-minimizer control of hereditary switched cross-degree energy and a
  joint speed-`n` tail, not another choice of order.
- The same quantity is exactly the restricted fourth-phase quadratic norm of
  `A+iR`.  Full operator-norm flattening would already force limiting value
  `1/2`, while exact sign-valued Clifford anticommutation is parity-impossible
  for every `n>=3`.  Independent random skew signs fail outright at
  exponential first-moment rate in the optimal regime: even the fictitious
  assignment with every central internal energy zero requires
  `m_n/n^(3/2)>sqrt(log(2)/2)=0.588705...`, while the known upper scale is
  `1/2+o(1)`.  Energy-distribution refinement cannot repair that union bound.
- Fixed-real-part Hermitian characteristic polynomials do form a valid
  one-sided interlacing family, with exact matching-transform average.  This
  is not the matching polynomial because signed `A`-cycles survive.  The
  two-sided spectral-radius upgrade fails already for `A=J-I` on `K_3`, and
  a full operator-norm estimate strong enough for the cube target would
  itself force `alpha_n->1/2`.  Do not cite Hermitian interlacing as a
  multiplier-two close.
- The literal recursive reuse of the same two real/imaginary halves is now
  excluded exactly. For `K=[[1,i],[-i,1]]`, the restricted fourth-phase norm
  obeys `nu_4(K tensor H)=4nu_4(H)`. Completing the paired edges changes this
  by at most `2n`, while the next doubling estimate permits only a
  `2sqrt(2)` factor. The gap is therefore positive of order `n^(3/2)`, not a
  Dini bookkeeping error. A general vertex bisection still leaves coupled
  cross-rectangle flips, so a changed premise must prove a new coupling for
  those terms rather than alter paired-edge signs. See
  `evidence/NOTE_2026-09-03_TWO_HALF_SELF_GLUING_OBSTRUCTION.md`.
- The cross-rectangle Fourier follow-up proves exact analytic stability and
  Gram rigidity and supplies an infinite Gram-perfect family attaining the
  spectral maximum. This retires norm-only and Gram-only merges. It does not
...
- A new exact direct equivalence is
  ...
  normalized complete equimodular homogeneous quadratics.  Consequently the
  original MO limit exists iff `K_n` converges.  Existing bounded-degree
  influence theorems bound `K_n` but give no comparison in `n`.  Exact
  one-step comparison has a nonsummable `O(n^(-1/2))` loss; random padding,
  ordinary graph blow-up, generic tensor relaxation, and weighted compactness
  all have proved scale/closure obstructions.  Do not repeat them.
- The order-six conference signing gives the hand-checkable bounded
  homogeneous quadratic `f=Q_A/5` with total `L^1` influence `9/4>2`.
  Its derivative convention exactly matches Filmus--Hatami--Keller--Lifshitz,
  so it refutes their literal `Inf_1(f)<=degree(f)` conjecture.  This is an
  independent theorem with no priority claim, not a solution of the MO limit.
- Proposition 6.10 gives the correct thermodynamic gate without guessing the
  value: for
  `s_n(c)=n^(-1) min_A log E cosh(c Q_A/sqrt(n))`, exactly
  `s_n(c)/c<=alpha_n<=s_n(c)/c+log(2)/c`.  Convergence on any unbounded set
  of `c`, or pressure oscillation `o(c)`, would prove the MO limit.  Standard
  interpolation does not supply it: equal splitting at common raw `beta`
  shifts `c` to `c/sqrt(2)` and leaves a per-spin gap tending to `c^2/8`.
  Ordinary graphon convergence is also insufficient.  For each `0<c<1`,
  conference and deterministic quasirandom sequences both have signed cut
  norm tending to zero but pressure rates at most `(1/2)log cosh(c)` and
  exactly `c^2/4`, respectively.  A live continuation needs an optimized
  disorder lower tail at speed `n^2` or a stronger second-order limit object.
- Proposition 6.10a rules out the literal conference-product lower curve at
  every positive temperature. The optimal order-five cycle/diagonal signing
  has exact partition function `(5u^2-1)/4<u^(5/2)` for
  `u=cosh(2c/sqrt(5))>1`. Its universal Gibbs-entropy fallback has
  large-temperature slope only `1/pi`, so it restates the known floor and
  does not settle Proposition 6.10's optimized pressure gate.
- The exact-value panel now runs through `n=15`.  A fresh unchanged replay of
  CurtisAccelerate's witness and 50,778,686-node lower-bound enumerator proves
  `m_11=17`; parity, monotonicity, and a checked witness then prove `m_12=18`.
  Published certificate packages give `m_13=20,m_14=21,m_15=27`; their
  lightweight checks passed, while their billion-record/tower completeness
  remains an explicitly recorded external trust boundary.  These add
  certified points, not an asymptotic inference.
- Replay:
  `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python scripts/original_mo_two_half_geometry.py --solve --workers 8 --max-time 180`
  and
  `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src:. pytest -q tests/test_original_mo_two_half_geometry.py tests/test_orientation_structure_audit.py tests/test_random_order_variance_gate.py tests/test_fixed_real_hermitian_interlacing.py tests/test_complexification_opposite_diagonal.py tests/test_bivector_energy_layer_minimax.py tests/test_signed_regular_arcsine_rigidity.py tests/test_finite_anchor_signature_tournament.py tests/test_random_skew_mate_second_moment.py tests/test_prop65h_directed_halfcut.py tests/test_gaussian_saturation_central_saddle.py tests/test_bivector_degree4_preordering_no_go.py tests/test_banaszczyk_weighted_anchor_rounding.py tests/test_bivector_growing_degree_preordering_no_go.py tests/test_conference_commuting_mate_no_go.py tests/test_coherent_clique_optimal_scale_counterfamily.py tests/test_original_mo_thermodynamic_gate.py tests/test_pressure_lower_curve_no_go.py tests/test_main_chain_docs.py`.
  The combined calculator, orientation, random-order, interlacing, arcsine,
  Gaussian-saturation, weighted-rounding, proof-system, conference-parity,
  coherent-counterfamily, and critical-pressure suites give 96 passing
  focused tests.
  SHA-256:
  `9e1d16c7de66e7e585b9a85593d1feeaeb3d217cf5e13bbfee1509bb1be78474`
  for the JSON and
  `c845a58d151b460fa63f3efb0479c5819cc02420692fd411946b54a53947f858`
  for the plot.
- Primary artifacts:
  `evidence/NOTE_2026-09-02_ORIGINAL_MO_TWO_HALF_GEOMETRY.md`,
  `evidence/NOTE_2026-09-02_FOURTH_PHASE_CLIFFORD_AUDIT.md`,
  `evidence/NOTE_2026-09-02_TWO_HALF_RANDOM_SKEW_CRITERION.md`,
  `evidence/NOTE_2026-09-02_CUT_CODE_SPHERICAL_LIMIT_AUDIT.md`,
  `evidence/NOTE_2026-09-02_ORIGINAL_MO_L1_INFLUENCE_CALCULUS.md`,
  `evidence/NOTE_2026-09-02_SHARP_INFLUENCE_TENSORIZATION_AUDIT.md`,
  `evidence/NOTE_2026-09-02_EQUIMODULAR_COMPACTNESS_NO_GO.md`,
  `evidence/NOTE_2026-09-02_BEST_RESPONSE_EDGE_FLIP_NO_GO.md`,
  `evidence/NOTE_2026-09-02_ORIENTATION_STRUCTURE_AUDIT.md`,
  `evidence/NOTE_2026-09-02_ORDERED_SKEW_PREFIX_HALF_CUT.md`,
  `evidence/NOTE_2026-09-02_HALF_CUT_LOCAL_MINIMAX_OBSTRUCTION.md`,
  `evidence/NOTE_2026-09-02_RANDOM_TOURNAMENT_PARTITION_THRESHOLD.md`,
  `evidence/NOTE_2026-09-02_RANDOM_ORDER_HALF_CUT_VARIANCE_GATE.md`,
  ...
  `evidence/NOTE_2026-09-02_BIVECTOR_DEGREE4_PREORDERING_NO_GO.md`,
  ...
  `evidence/NOTE_2026-09-02_DIRECTED_HALF_CUT_LITERATURE.md`,
  `evidence/NOTE_2026-09-02_DIRECT_LIMIT_LITERATURE_AUDIT.md`,
  `evidence/NOTE_2026-09-02_EXTERNAL_M11_CERTIFICATE_AUDIT.md`, and
  `evidence/NOTE_2026-09-02_EXTERNAL_N12_N15_CERTIFICATE_AUDIT.md`.
- **Status remains L OPEN.**  A finite plot cannot certify convergence; it
  has isolated an exact all-orders invariant and ruled out two seductive
  false shortcuts.

## Current working checkpoint (2026-09-04; audited Proposition 15.773 and post-15.761 exact reductions)
...
- Propositions 15.757--15.761 continue the separately audited
  ...
  Proposition 15.757 computes
  `rank_F2 R_2=p^2-1+(p+1)binom(p-1,2)` and proves that boundary plus total
  parity are the complete binary conditions. The recorded `p=1 mod 4`
  compact aggregate family passes them, so a binary lift is not a simple
  integral lift.
- Proposition 15.758 proves the exact fixed-offset coefficient minimum
  `|3a-b|`, except for the value `3` when `|3a-b|=1`. It constructs local
  survivor rays for both `p=1 mod 4` and `p=3 mod 4` across their full
  isolated-chart intervals. Their compact row-energy upper bounds exceed the
  scalar common-Parseval lower bounds from `r>=7`; these are local rows, not
  a common graph.
- Proposition 15.759 gives every characteristic-`p` edge--Radon moment row
  and proves independence/exhaustion, with extra codimension
  `S(p)=(m-1)(4m^2+7m+6)/6`. Proposition 15.760 proves the exact integral
  image theorem `A/R(E)=(Z/pZ)^S(p)`. Hence those moments plus the ordinary
  totals are sufficient for an unrestricted signed integral lift, but not
  for a nonnegative `0/1` lift.
- Proposition 15.761 computes all three real spectral blocks of `RR^t` and
...
- Propositions 15.768--15.770 close the first two generic post-band layers.
  For every prime `p=1 mod 4`, `p>=29`, the closed rows are
  `t=(p-7)/2,k=5p-7` and `t=(p-5)/2,k=5p-5`.  For every prime
  `p=3 mod 4`, `p>=31`, they are `t=(p-5)/2,k=5p-5` and
  `t=(p-3)/2,k=5p-3`.  The proofs use the isolated-chart residue ledger,
  common-row normalization, sharp `p-3` lift classification, and the fixed
  four-bit Boolean catalog.  The one-row carry in 15.770 advances each
  classified equality family without a new graph, orbit, slice, or residual-
  candidate census.
- Proposition 15.769 also closes the exceptional endpoint
  `p=23,t=9,k=110`.  The local mass-36 cell has height three, every paired
  cube has mean `1/2`, and the equality forms are exactly `F_4` and `F_5`.
  Vanishing additive `2x2` minors globalize them on `J(23,12)`; coefficient
  compatibility leaves the `F_5` complete graph on five points.  Twelve hard
  roots force the quartic/octic identities, but among all 33,649 five-sets
  exactly 1,518 satisfy the quartic equation, 2,024 satisfy the octic one,
  and zero satisfy both.  This is a fixed coefficient certificate, not a
  graph census.
- Proposition 15.770 also closes `p=23,t=10,k=112`.  Its one-row carry has
  eleven low hard triangle-minus-star roots, enough to annihilate the same
  degree-four and degree-eight forms.  The corrected opposite surplus is
  four, so at least eight mass-36 rows occur; coefficient compatibility
  again forces `P=4,Q=5,F_5`, contradicting the same fixed 33,649-five-set
...
- With `q=(p-1)/2` and `k=4p+2t`, the exact residual-(ii) frontier is
  ...
  `p=23,t>=12`; `p=1 mod 4,p>=29,t>=q+3`;
...
- The higher-moment follow-up proves that every odd row is blind on the
  ...
  all odd global forms vanish and `3b<=r+2`. For balanced branch C, writing
  `delta=t-(2r^2-4r-2)`, every opposite row is central throughout
  `0<=delta<=(2r+2)floor((r+2)/3)`. This is structural only and does not cover
  nonzero odd forms or unbalanced allocations.
- At `p=31` the original theorem gave centrality through `68<=t<=116`; the
  frozen component-packing theorem below supersedes this by the guaranteed
  balanced opposite-row band `68<=t<=164`. The exact 450-orbit
  classification itself handles the local
  one-compact/six-all-equal row: 449 noncentered fibers are `UNSAT`, the unique
  centered orbit at index 435 is covered by the prior exhaustive theorem, and
  there is no `SAT` fiber in 317,916,856 DFS states. Every balanced profile
  with `69<=t<=99` contains at least one such row, so zero odd, degree-six, and
  degree-eight global forms are impossible across that band. This does not
  cover unbalanced allocations or nonzero forms. The v2 source, full log,
  corrected merge record, and independent v1/v2 overlap are hash-pinned in
  the compact-ray note.
- Exact seven-channel Jacobian minors for four compact and four all-equal atoms
  are nonzero, so both degree-six/eight atom maps are dominant over the
  algebraic closure in characteristic at least 11. This kills a universal
  polynomial identity or purely algebraic projective root-count close. The
  construction uses labels and common forms over an algebraic closure or finite
  extension; it supplies neither `F_p` labels/form coefficients, compatibility
  with odd or higher moments, nor a Boolean lift.
- The full balanced `p=4r+3`, `r>=7`, support audit now excludes every word
  containing `h=2r+1` collinear points: Couvreur peeling leaves one or two
  maximal lines, and both configurations fail exact coefficient, parity, or
  aligned-capacity bounds for every `0<=b<=r`. At the boundary `b=r`, every
  boundary cubic/degree-`h-2` complete intersection is also excluded for
  `p=4r+3>=31`, covering reducible, integral singular, and smooth cubics.
  Neither theorem eliminates the remaining high-intersection irreducible
  conic or extends the earlier centrality interval by itself.
- Conic peeling forces that remaining word wholly onto a triangle-tangent
  conic with constant orbit difference. The star is parity-impossible; every
  nonstar survivor satisfies `q^3=1`, `q!=1`, equivalently `k^2=-3`, and
  hence `p=7 mod 12`. The exact `p=31,b=7,k=11` six-AE/seven-compact witness
  realizes all 29 conic edges and kills 105 odd channels, but has
  `F6=(11,19,10)` and `F8=(12,11,23,6)`. The separate MITM certificate
  exhausts the entire simultaneous zero-six/eight fiber: 230,314,710 maximal
  completions, 17,076 edge hits, and zero moment hits. This is exact `UNSAT`
  for that finite fiber only.
- For an integral lift `Rz=y`, the signed Boolean gate is exactly
  `beta_R(y)=0`, with `beta(z)=(||z||_2^2-H_y)/2`. A lift minimizes this
  defect exactly when `|2 z.g|<=||g||_2^2` for every element of the complete
  Graver basis. The proved ridge circuits give only the exact reduction
  `p ker_Z R subset K_ridge subset ker_Z R` and
  `ker_Z R/K_ridge=(Z/pZ)^nu_p`, where, for `m=(p-1)/2`, `d=p+1`,
  `nu_p=d p m^2+m(m-1)(4m+1)/6`. The ridge lattice is proper; these results
  neither supply the complete Graver basis nor establish a Boolean lift.
- The exact equianharmonic pairing-component classification proves the
  necessary threshold `b>=(2r+7)/3` for the constant conic, for both signs.
  Consequently, under zero odd global forms, every opposite row is central
  when `3b<=2r+4`; if `p=11 mod 12`, it is central for all `b<=r`. The
  `p=43,b=9` witness attains the threshold and kills all 210 odd channels,
  but its `F6,F8` syndromes are nonzero. This is a necessary threshold, not
  sufficiency or a residual close.
- Every balanced hard row is one fixed unit star plus `e<=2r-2` compact
  atoms. Under zero odd forms its compact residual is central over the
  integers; the whole row retains the unit-star antisymmetry. Central
  inversion has antisymmetric ranks `d^2 h^2`, `d h^2`, `d p h^2` and
  cokernel `(Z/pZ)^[h(h-1)(h+1)/3]`, exactly the odd moments. Explicit
  direction-localized Mobius trades, chosen disjointly by a sharp greedy
  bound, realize arbitrary hard-star centers. The **antisymmetric Boolean
  half is therefore closed** for the balanced zero-odd target.
- The live Boolean gate is only the coupled symmetric half: pair totals are
  forced to `s_e=1` on used Mobius orbits, may be `0` or `2` on unused
  nonfixed orbits, and are independent binary choices on fixed antipodal
  edges. No common simple graph is constructed. The earlier hard-star
  support/equality/total/parallel/norm diagnostics are superseded; do not
  rerun them.
- At the equianharmonic threshold, exact `U,V` four-compact/two-cycle trades
  preserve every odd edge channel. Their mixed degree-six/eight syndrome map
  has Jacobian `2^32*3^26*5^2*7*2161` at a rational point, so a
  component-only affine even-syndrome obstruction cannot work. This is a
  characteristic-zero dominance barrier, not a finite-field zero witness.
- Frozen evidence is in
  ...
  `evidence/NOTE_2026-09-03_EQUIANHARMONIC_THRESHOLD_EVEN_BARRIER.md`.
- The symmetric plus-lattice has ranks `d h(p^2+1)/2`, `d h(h+1)`, and
  `d p h^2`; its mod-two map is surjective and its exact cokernel is
  `(Z/pZ)^[(h-1)(2h^2+5h+6)/6]`, exactly the even moments. This supplies an
  unrestricted integral central lift when even moments vanish and a mod-two
  central lift for every compatible target, not the restricted central Boolean lift.
- One Mobius half has exact parallel counts
  `1,1,0,1+eta(1+m)` in rows `L,M,L-M,L+mM`; its Paley signs are the
  quartic trace `tau_t=eta(Q(e1-t^2(e1+e2)))`, and its forced symmetric
  pair chain doubles the parallel vector. Two arbitrary nonzero hard-star
  trades can cancel on exactly one shared origin orbit, leaving
  `2(p-1)-2` nonzero orbits. This corrects the false inference from the
  disjoint count `m(p-1)=|H|_max+1`; there is no one-edge capacity close.
- If every hard center is nonzero, the equality-pencil and prime-order
  Redei--Megyesi direction theorem exclude `c=p-2`, so `c>=p-1`. This
  all-active balanced zero-odd lemma does not cover zero centers and does not
  solve the symmetric Boolean box. Frozen notes:
...
- Fixed antipodal variables in that box are now eliminated exactly. The
  block map is `R+=[[A,2B],[0,C]]`; `A mod 2` is an isomorphism with explicit
  inverse `a_[v]=g_(L_v)(0)+sum_L g_L(L(v)^2)`. After subtracting the unique
  forced word and dividing its fixed block, the live variables are precisely
  the unused nonfixed orbit bits. They obey
  `2 sum b=|H|-|U|-|a|` and one exact integer weight slice per direction.
- For a used orbit `(a,[delta])`, the forced-word change `Phi` is zero when
  `a||delta` and otherwise is the `p`-point antipodal affine block
  `{[delta+c a]}`. These blocks satisfy `M M^T=M^T M=I` over `F_2` and have
  `p`-column lifts through `ker C`. Hence the full halved map `D=(C,Phi)` is
  onto, of rank `d h(h+1)`. The punctured map is onto exactly when `U`
  contains no support of a nonzero word in `Row(D)`; particular-target
  membership is the corresponding left-kernel pairing test.
...
- The corrected all-active capacity demand is
  `kappa>=t_max-t+1` when `|U|=m(p-1)-2 kappa`. For `p=31` it ranges from
  110 at `t=68` to one at `t=177`, so the disjoint lift is never extendable
  on the ray. Two halves cancel on at most two shared orbits, sharply, but
  the sharp locus is rigid (`q=r=1/2,A=B=3/2`) and gives no greedy parameter.
  The exact first test is
  `|U|+|a_Y+sum_(O in U) Phi(O)|<=|H|`; the divided Boolean fibre still
  follows it. Frozen notes are
...
- The adaptive kernel-selector product now closes the remaining
  opposite-fixed top `j=0` localized-Mobius family for every applicable
  `p=4r+3`.  Here `t=t_max`, `s=m+2`, `m=(p+1)/2`, and
  `q_(L,M,c)=boundary(E_(L,M,c))+1_(L=c)`.  If `sigma_K=(-1)^g_K` is its
  pairing with zero plus one representative of every nonzero antipodal pair
  in `ker K`, then, for `D(z)=M+z(L-M)`,
  `sigma_L=-1`, `sigma_(L-M)=epsilon_L`, and
  `sigma_(D(z))=epsilon_(D(z^2))` for finite `z!=1`.  Therefore
  `product_K sigma_K=-epsilon_M`.  The forced auxiliary SDR has `m-2` hard
  and two opposite directions, so the `m`-half product is `+1`.  Every valid
  ternary correction on a nonorigin inversion orbit changes two kernel bits;
  the SDR prevents origin-orbit corrections, while the single fixed edge
  changes one bit.  The final product is therefore `-1`, forcing an adaptive
  kernel contradiction regardless of higher nonorigin overlaps.  This is a
  family closure, not a residual-(ii) or arbitrary-lift closure.
- The independent `p=31` replay checked 14,880 half options, 446,400 center
  instances, and 14,284,800 selector pairings with zero failures.  The global
  artifact
  `evidence/e1_gmin_m4_p31_top_mobius_boundary_parity_global_replay.json`
  has SHA-256
  `9ce413b62c65b7a541388f49bb390c690af44389556e6e8676ec138ef2cbc533`.
  The bounded-component artifact
  `evidence/e1_gmin_m4_p31_top_mobius_boundary_parity_component_replay.json`
  witnessed all 2,969 searchable designs
  (minimum seven adaptive directions, no universal fixed direction) and has
  SHA-256
...
- Do not carry the `j=0` conclusion into `j=1`.  Preserve both formal Hamming
  alternatives: `f=3,d=0`, including the origin-collision branch, and the
  conditional `f=1,d=1` branch.  The immediate target is the full
  degree-boundary/compact-atom synchronization across both, not another
  fixed-`F` selector search and not a QR/BCH route.
- Two older apparent automatic equality closes are ruled out exactly and do
  not bypass the adaptive-selector theorem.
  ...
  family adaptively from the centers in the remaining branches; that broader
  problem remains open. The
...
- Adaptive centre coherence is proved at the target-pair level for every
  ...
  `evidence/NOTE_2026-09-03_ADAPTIVE_MOBIUS_PAIRING.md`.  This route is
  superseded as a target for the closed opposite-fixed top `j=0` family.
  Replay the older matching result with
...
- The branch-C atoms are now coupled to the actual Mobius support at the
  ...
  surviving scalar bounds alone do not give a construction.  For `j=1`, the
  full degree boundary, compact atoms, even moments, and nonfixed target cells
  must now be synchronized across both Hamming alternatives. See
...
- At `p=31`, cancellation parity is now fully audited. The 16 halves start
  ...
  `|a(T_U)|` is odd while the Hamming numerator is automatically even. At
  `kappa=178-t`, any completion is forced to one fixed antipodal edge and
  zero unused double orbits. This is a conditional consequence, not a
  completion or contradiction. Zero-`Phi` parallel means midpoint parallel
  to difference, not the directionwise parallel-edge statistic.
- The sharp rigid pair has source-`Phi` weight
  `4p-26-2 eta(3)-4 eta(6)-4 eta(-2)` (108 at `p=31`), but do not reuse the
  retracted target identity `a_Y=a_literal`. The correct identity is
  `a_Y=a_literal+a_compact`; a central compact residual can have odd fixed
  cells, so the rigid source word supplies no Hamming close.
...
- The prescribed-center common-block problem has an exact equal-square Hall
  ...
  fixed `K` only. Conversely, for branch primes `p>=31` with all hard centers
  nonzero, a prescribed anchor line whose link graph is a pseudoforest gives
  a saturated equal-square incidence cover. Existence of
  such a line, mutual ternarity, full dual-support containment, and the
  Boolean completion are all still open. The additional frozen notes are
...
- The ordered live gate is exact and narrower. The p31 result closes only the
  balanced `69<=t<=99` zero-global-form odd/degree-six/degree-eight band.
  Every balanced opposite row is zero-odd central through `t=164`, and the
  antisymmetric box is solved. Remaining branches include unbalanced
  zero-form allocations, simultaneous nonzero even global forms over `F_p`
  coordinated across directions, and the coupled symmetric pair-total box.
  Residual (ii), E1, and `L` remain OPEN.
- Focused replay:
  `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src:. /home/nick/.venvs/mo-exact/bin/python -m pytest -q tests/test_prop15757.py tests/test_prop15758.py tests/test_prop15759.py tests/test_prop15760.py tests/test_prop15761.py tests/test_prop15763.py tests/test_prop15768.py tests/test_prop15769.py tests/test_prop15770.py tests/test_p23_post_band_moment_close.py tests/test_p23_second_post_band_moment_close.py tests/test_compact_ray_moment_gate.py tests/test_conic_odd_radon.py tests/test_p31_equi_zero68_mitm.py tests/test_signed_boolean_defect.py tests/test_ridge_kernel.py tests/test_equianharmonic_component_packing.py tests/test_hard_compact_odd_radon.py tests/test_hard_star_antisymmetric_support.py tests/test_inversion_antisymmetric_radon.py tests/test_equianharmonic_threshold_even_barrier.py tests/test_inversion_symmetric_lattice.py tests/test_mobius_half_symmetric.py tests/test_all_active_pencil_support.py tests/test_symmetric_fixed_edge_elimination.py tests/test_symmetric_halved_mod2.py tests/test_symmetric_halved_mobius_cover.py tests/test_symmetric_slice_exchange.py tests/test_mobius_half_intersections.py tests/test_symmetric_halved_row_code.py tests/test_prescribed_center_common_block.py tests/test_rigid_pair_fixed_word.py tests/test_grouped_uncertainty_square.py tests/test_symmetric_halved_row_code_gap.py tests/test_symmetric_quota_cardinality_barrier.py tests/test_p31_top_mobius_boundary_parity.py tests/test_main_chain_docs.py`.
  Residual (ii), E1, `L=1/2`, and the original MO limit remain OPEN.
  The compact-ray file independently passes 22 focused tests. Its exact
  theorem implementation and evidence are
  `src/e1_gmin_m4_compact_ray_moment_gate.py` and
  `evidence/NOTE_2026-09-02_COMPACT_RAY_HIGHER_MOMENT_GATE.md`; the preceding
  checkpoint's exact theorem records are
  `evidence/e1_gmin_m4_prop15757.json` through
  `evidence/e1_gmin_m4_prop15761.json`.  The post-band theorem records are
  `evidence/e1_gmin_m4_prop15768.json` through
  `evidence/e1_gmin_m4_prop15770.json`, with the exceptional p23 certificate
  in `evidence/e1_gmin_m4_p23_post_band_moment_close.json`.

The inputs immediately preceding this checkpoint are:

- Proposition 15.755 is a full-Boolean-cube reduction for any hypothetical
  dangerous residual descent.  A shared maximizer has defect `2p` or, for
  `p>=11`, at least `6p-12`.  Both bounds are sharp for every prime: affine
  parallel-fibre aliases populate the first shell and positive-triangle
  three-flips populate the second.  Sharing the `A/B` maximizer adds no
  independent hereditary cut inequality.  Do not retry a one-bit-only or
  lattice-gap-only close.
- Proposition 15.756 proves that the arbitrary-boundary character/Weil cap
  is vacuous for every even boundary size at least four.  The union of two
  parallel affine lines attains the spectral cap.  Ordinary directional
  Parseval is the same pair count.  Do not retry a D-only large-boundary
  cancellation argument.
- Those propositions are proved reductions and method barriers, not residual
  closure; the predicate stays False.
- Focused replay:
  `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src pytest -q tests/test_prop15755.py tests/test_prop15756.py`.

## Previous working checkpoint (2026-09-02; Proposition 15.754)

- Proposition 15.754 closes the sole fifth-shell remainder
  `p=13,k=60,u=6` by an exact finite aggregate/common-form certificate.
  Together with Propositions 15.752--15.753, this closes the whole fifth
  shell `k=4p+8` for every prime `p>=13`.
- The seven quotient partitions of five are exhausted by exact
  translated-cut row tables. Common `U=hM2` and `G=hM4-M2^2` tables close
  the two- and three-root cases, adjoining `J6=h(M6-M2^3)` closes the
  four-root cases, and sharp collision/row-energy ledgers close the high-root
  cases. This is
  not a graph, orbit, or common-realization census.
- The support implementation and records are
  ...
  `scripts/p13_u6_four_root_ugj.py`, and the three
  `evidence/e1_gmin_m4_prop15754_*` common-form JSON files. The exact main
  replay is
  `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src /home/nick/.venvs/mo-exact/bin/python -m e1_gmin_m4_prop15754`;
  its evidence SHA-256 is
  `f3c54009c8d12494aae14885d268f54dd89cffdd5a3730a766e3730e3b7e2f63`.
  The 32-test focused replay is recorded in
  `evidence/NOTE_2026-09-02_P13_U6_COMMON_FORM_CLOSE.md`.
- **Scope remained open globally at this checkpoint:** residual (ii), E1,
  and the limit were not closed.  Propositions 15.768--15.770 now supersede
  its first two generic post-band opens and close `p=23,t=9,10`; the
  current remainder is stated in the checkpoint above. The positive
  `p=7,z=7` line-boundary remainder remains separately tracked where the
  boundary ledger is in scope.

## Latest delivered checkpoint (2026-09-01)

- Commit `01942477f21e116a7f3de2112c6a6a0e0184e749` is on `main` and was
  pushed to `origin/main`. At that delivered stage, Propositions
  15.752--15.753 closed the fifth residual shell `k=4p+8` for every prime
  `p>=17`; Proposition 15.754 now adds the p13 endpoint. Neither checkpoint
  closes the global residual-(ii) predicate or the original MathOverflow
  convergence problem.
...
- For the **original MO convergence question**, the next serious target is
  ...
  next p13 row was `p=13,k=60,u=6`; Proposition 15.754 now closes that row,
  without solving the MO question. The open positive `p=7,z=7`
  semigroup/quotient campaign is
...
`c=3` fallback. Proposition 6.10 gives the correct optimized
critical-pressure sufficient gate, while proving that common-`beta`
interpolation and ordinary graphon convergence do not establish it.
Proposition 6.10a disproves the literal conference-product lower curve for
every positive temperature at order five; its universal entropy replacement
has only the known `1/pi` limiting slope. Proposition 6.5n independently
shows that bare `Phi(A)=Theta(n^(3/2))` control cannot prove the coherent
clique-flip estimate, while leaving its global-minimizer specialization open.
Two
additional route obstructions are now binding. Paley
conference maximizers show that (6.20) cannot be emptied from Hamming and
joint-energy data alone; doubling needs the global-minimizer property or a
finer `A`-dependent `R`. Alternating paired states show that fixed/mildly
growing anchor refinement cannot empty (6.42)--(6.43) using the current
spectral product bound; `1:2` needs an actual state-energy correlation or a
different cross block. The multiplier-two residue remains the immediate
gate; after it, the two-state `1:2` residue is the preferred
...
applicable proposition. Proposition 15.754 supersedes every earlier line
listing `p=13,k=60,u=6` as open; Proposition 15.753 supersedes the p17/p19
fifth-shell endpoint opens; Proposition 15.752 supersedes earlier lines listing
its displayed `p>=23,t>=4` band as open; Proposition 15.751 supersedes every
earlier line listing generic branch-B `t=3` as open; Proposition 15.750
supersedes every earlier Type-I status line. Propositions 15.768--15.770
supersede every earlier line that treats the first two generic post-band
...
**Date:** 2026-09-04 (through Propositions 6.3--6.10a and audited 15.773; required bi-tight levels 2 and 3 and multi-level Type I are
...
prime `p>=13`, Proposition 15.752's contiguous higher band, and the exceptional
...
`p=13,17,19,t>=5`, `p=23,t>=12`, `p=1 mod 4,p>=29,t>=q+3`,
`p=3 mod 4,p>=31,t>=q+3`, and the positive `p=7,z=7` branch)
**The quadratic-minmax limit is still OPEN.**
**Repo:** https://github.com/luckyseoul/quadratic-minmax-limit
**Active research branch:** `residual/p13-u6-common-moments` in
`/home/nick/quadratic-minmax-limit-residual-ii`; `main` is the integration
branch, not this checkpoint's current HEAD.
...
assigned proposition through 15.774 and maps the former scratch scripts to
...
height one. Proposition 15.752 closes `k=4p+8` for every `p>=23` and its
explicit contiguous higher band. Proposition 15.753 closes the p17/p19
fifth-shell endpoints, and Proposition 15.754 closes `p=13,k=60,u=6`, so
the fifth shell is closed for every `p>=13`. Propositions 15.768--15.770
then close the first two generic post-band layers and the exceptional
...
`p=13,17,19,t>=5`, `p=23,t>=12`, `p=1 mod 4,p>=29,t>=q+3`, and
`p=3 mod 4,p>=31,t>=q+3`. Propositions
...
Proposition 15.743 then closes the first generic large-prime resonance at
`p=17,k=74` by the exact `767<1211+34C` common-energy contradiction. Thus
those two rows are closed. Proposition 15.744 next closes `p=13,t=4,u=3`
by a six-root quartic after rebuilding the changed `|H|=61` local premise;
15.745 closes `u=0` by the exact `695<719` collision-one energy
contradiction. Proposition 15.746 classifies every sharp mass-ten hard cell
in `u=4` as one of 78 omitted pairs or 286 all-equal triples. Proposition
15.747 then uses an exact cut second moment and two height-four models to
...
At the 15.749 stage the `p=13,k=60` row remained open exactly at `u=6`;
15.754 now closes it. The later layers just listed remain open. Aut-Schur / Gsum /
...
for both signs. This is not all of the projective size-eight case: 15.713--
15.717 close positive infinity-plus-seven at `z=0,1,2,3`, while 15.718--
15.719 reduce and identify projected high-catalog structure at `z=7` without
...
## 6.5--6.10a — exact direct amplification residues
...
endpoint selection, an independent cross budget, a geometry-only emptiness
claim, or finite pair census. Exact central Paley conference maximizers prove
that those coarse data alone are insufficient; use global minimization or
finer `A`-dependent structure. The
...
soft-max equivalent to the original problem up to `log(2)/c_n` unless it
adds a uniform new estimate. Common-`beta` centered Fekete inequalities and
finite-temperature cavity monotonicity do not control `beta=c/sqrt(n)`.

Proposition 6.5m rules out the exact conference spectral shortcut inside the
doubling route.  At every even order and for every symmetric/skew signing
pair, `(AR-RA)_ii=2 (mod 4)` for all `i`, so exact commutation is impossible
and `||AR-RA||_F^2>=4n`.  This remains compatible with the approximate
`o(n^4)` rigidity target and therefore does not close or refute the outgoing-
half orientation inequality.

Proposition 6.5n supplies an asymptotic no-go for the coherent branch. Take
`b` Hadamard saddle gadgets of order `4r`, complete all inter-gadget edges
with one low-norm random-sign filler, and choose its global sign to preserve
the union of the clique witnesses. With `r=256b`, `b=2^j`, the resulting
complete signings satisfy `Phi(A)=Theta(n^(3/2))` but
`liminf (L_cl(A)-sqrt(2)Phi(A))/n^(3/2)>=1.1657...`. Thus no scale-only
argument proves coherent control, even asymptotically. This is not a
counterexample for global minimizers: the matrices have a deliberately large
leading constant, and `Phi(A)=m_n => L_cl(A)<=sqrt(2)m_n+o_Dini(n^(3/2))`
remains open. The noncoherent `A`-dependent branch also remains open.

Proposition 6.10 defines the optimized critical pressure `s_n(c)` and proves
the exact entropy sandwich
`s_n(c)/c<=alpha_n<=s_n(c)/c+log(2)/c`.  This is a valid sufficient route to
the MO limit on any unbounded temperature set.  The standard block inequality
has a limiting `c^2/8` equal-split defect and a `c -> c/sqrt(2)` parameter
shift.  More strongly, signed cut convergence to zero does not determine the
critical pressure: conference signings have rate at most
`(1/2)log cosh(c)`, while a deterministic quasirandom sequence has rate
`c^2/4`, for every `0<c<1`.  Do not cite ordinary graphon/right convergence
or the quenched Guerra--Toninelli limit as control of the outer optimized
minimum.  The open thermodynamic input is a speed-`n^2` optimized Bernoulli
lower tail or a finer second-order limit object.

Proposition 6.10a blocks the proposed direct replacement
`E cosh(cQ_A/sqrt(n))>=cosh(c sqrt(1-1/n))^(n/2)`. For the optimal
order-five signing negative on a five-cycle and positive on its diagonals,
the energy histogram is `{5,6,5}` at `{-4,0,4}`. Hence its partition
function is `(5u^2-1)/4`, while the proposed curve is `u^(5/2)`, and the
difference factors positively for every `u=cosh(2c/sqrt(5))>1`. The exact
Gibbs variational fallback
`sup_r {c r^2 Phi(A)/n^(3/2)-I(r)-log(2)/n}` is valid universally, but after
the Proposition 5.2 input its large-`c` slope is only `1/pi`. It is a genuine
lower curve, not progress toward the missing `1/2` lower bound, and does not
refute the optimized gate of Proposition 6.10.

## 15.754 — common-form certificate closes the p13 fifth-shell endpoint

At `p=13,k=60,u=6`, common quotient arithmetic forces seven hard excesses
with partition `sum k_L=5`, common normalization `P_0=4,hT=5`, and opposite
parallel count `Q=4`. The seven partitions of five split by the number of
exact XNOR roots. Exact row-energy tables joined through `U=hM2` and
`G=hM4-M2^2` exclude the two- and three-root partitions. Adding
`J6=h(M6-M2^3)` excludes the two four-root partitions. For the remaining
high-root partitions, the common quartic vanishes and sharp
collision-dependent row maxima fall strictly below the common Parseval
energy.

All joins range over the full exact aggregate alphabets and both hard signs.
The supporting code and records are `src/p13_u6_cut_equalities.py`,
`src/p13_u6_high_root_energy.py`, `scripts/p13_u6_joint_ug_tables.py`,
`scripts/p13_u6_low_root_ug_bound.py`,
`scripts/p13_u6_four_root_ugj.py`, and
`evidence/e1_gmin_m4_prop15754_{low_root_ug,four_root_221,four_root_311}.json`.
This is an exhaustive finite aggregate/common-form certificate and a proved
endpoint theorem, not a graph, orbit, or common-realization census.

Thus `p=13,k=60,u=6` is closed. Together with 15.752--15.753, the fifth
shell `k=4p+8` is closed for every prime `p>=13`. Residual (ii), E1, and the
limit remain globally open. Propositions 15.768--15.770 later close the first
two generic post-band layers and `p=23,t=9,10`; see the current checkpoint
above for the remaining frontier.

## 15.753 — translated-cut energy closes the p17/p19 fifth-shell endpoints

At `t=4`, exact floor and quotient-sum ledgers leave A/B at `p=17` and A/C
at `p=19`. The common row-sum identity is applied before optimization, so
the four branches have signed totals `hT=5,21,5,-15` and forced parallel
normalizations `P_L=4+k_L` or `3+k_L`. All 698 p17 and 2,338 p19
translation-averaged cuts are imposed in exact one-worker row models.

Every XNOR excess partition has a strict common-Parseval gap. The last p17
partition needs eight exact roots of `hM4-M2^2`; opposite normalization gives
`S4=-S2^2`, yielding `384+9*11=483<645+34C`. The literal branches have
five exact roots, force `M2=M4=0`, and are empty or energy-deficient. Thus
`p=17,k=76` and `p=19,k=84` are closed. This is a deliberately narrow
finite-prime aggregate certificate, not a graph or common-realization
census; the global residual predicate remains false.

## 15.752 — influence rigidity closes the fifth shell and a higher band

For every prime `p>=23`, a nonzero nonnegative integral quadratic on
`J(p,(p+1)/2)` cannot have `4p E[B]=p+9`. Paired-cube averaging and the
sharp stabilizer bound reduce height at least two to 15.751's half-mean cube
theorem. At height one, the corrected transposition-influence bound leaves
at most six relevant coordinates, cube influence reduces this to four, and
the fixed 222-table catalog misses density `(p+9)/(4p)`.

In the isolated chart, branches A/C force that forbidden mass and branch B
forces 15.751's forbidden mass `p+7`. Thus `k=4p+8` is empty for every
`p>=23`, and so is the whole band `4<=t<=(p-9)/2` for `p=1 mod 4` or
`4<=t<=(p-7)/2` for `p=3 mod 4`. This is a proved infinite-family theorem,
not a prime/configuration census. The local `p=19` threshold has explicit
quadratic witnesses, so its local lemma cannot cover the endpoint.
Proposition 15.753 closes `p=17,k=76` and `p=19,k=84` instead by exact
common-energy certificates.

## 15.751 — influence rigidity closes the fourth shell

For every prime `p>=13`, `k=4p+6` is empty. The generic `p>=29` branch is
closed by a dimension-free half-mean cube height theorem followed, in the
Boolean case, by corrected transposition influences and the fixed four-bit
catalog. Prior exact certificates supply `p=13,17`. This is an
infinite-family theorem with one fixed exhaustive certificate; do not reopen
generic `t=3` with a prime, graph, orbit, slice, or cell census.
...
`u=6`; Proposition 15.754 later closes it.
...
and the exact weighted-graph second moment force

```text
-7D2+84E2+182Q^2-1428Q+2598=0.
```

The residual is always one modulo seven, so height one is impossible for
every integral `Q`. Deterministic one-worker necessary-relaxation models
also exclude height four at the live counts `Q=3,5` (169 variables and
3,526 constraints each). Therefore the omitted-pair `P=3` branch is closed,
and every minimum `Q=3` cell in the all-equal-triple `P=5` branch is the
exact `b=12` literal.

If `z` is the number of those literals, their directions are common roots of
`M2,M4,M6`. Root counting excludes `z>=5`; exact interpolation against the
69-element hard moment alphabet excludes `z=4,3` for both hard signs. At
`z=2`, list recovery leaves exactly 336 moment-level survivors per sign.
...
The hard `u=4` cells have `b=2` and scaled mean 22.  First use the
all-positive `b=2` quadrature to identify the XNOR baseline pointwise; only
then does Proposition 15.688 apply to the globally nonnegative difference.
It is Boolean of support 330 on `J(13,7)`.

Reuse 15.738's rank-78 quadratic evaluation space and all 1,638 independent
third-difference identities.  The exact no-new-support model has 1,716
Boolean variables and 1,710 constraints: the identities, one support
equation, one anchor equation, and 70 anchored candidate no-goods.  The
catalog and anchor reductions are checked under generators of `S_13`.
Exact infeasibility, with an independent deterministic one-worker replay,
proves the complete catalog:

```text
78  omitted pairs       (1-x_i)(1-x_j)
286 all-equal triples   1-x_i-x_j-x_k+x_i*x_j+x_i*x_k+x_j*x_k.
```

The two catalog SHA-256 digests are
...
for the full and anchored lists.

The two families have total coefficient offsets three and five.  Common
`hT`, the isolated-chart congruence, and `7P<=61` force `P=3` for every
omitted-pair hard cell or `P=5` for every all-equal-triple hard cell; mixing
is impossible, and `hT=14P-61`.  For the seven opposite directions,

```text
e_L=P+Q_L-8 >= 0,   sum e_L=5,   a_L=12+14e_L.
```

Thus at least two opposite cells have mean 12.  In the `P=3` branch they
have `Q=5`; the exact `b=12` literal is impossible modulo six, so each is a
`b=0` mass-12 lift of height one/support 396 or height four.  In the
`P=5,Q=3` branch, the literal-or-lift dichotomy survives.

For omitted-pair hard cells, the normalized moments obey the two-power-sum
Newton identity

```text
F6 = 2hM6 + hM2^3 - 3M2M4 = 0.
```

The certificate checks every one of the `78^2=6,084` baseline/lift overlap
cases and the degree-two, four, and six gauge cancellations.  Seven hard
roots make this binary sextic identically zero.  Hence every forced
`P=3,Q=5,b=0` mass-12 opposite cell satisfies `F6=0`.  For `P=5`, the 22,308
pair/triple patterns give full weighted feature ranks `1,2,3` through degree
six, so there is no analogous universal polynomial identity in `N2,N4,N6`
at those degrees; no broader invariant is ruled out.

For the opposite-cell normalization `N'_(2r)=(-h)M_(2r)`, impose this as
`2N'_6+(N'_2)^3+3N'_2*N'_4=0`; the mixed term changes sign relative to the
hard-sign display.  Do not copy the hard-cell Newton formula directly into
...
`u=4` branch; at that stage the exact `p=13,k=60` remainder was `u=6`,
which Proposition 15.754 later closes.

## 15.745 — collision-one energy closes `p=13,t=4,u=0`

The seven hard quotients have excess partition `(4)`, `(3,1)`, `(2,2)`,
`(2,1,1)`, or `(1,1,1,1)`.  Gluing an exact literal star to the common
difference transform gives `hT=17` and `P_L=4+k_L`; the opposite profile is
`3^6,4`.  Three exact directions force `M_2=0`, and five force
`M_2=M_4=0`.

All 74 translated seven-set cuts give exact necessary six-bin row
certificates.  With both moments, an opposite `Q=3` row is infeasible,
closing the first three partitions.  With `M_2` alone, the sharp relaxation
maxima are `31,96,76,111` for hard `e=1`, hard `e=2`, opposite `Q=3`, and
opposite `Q=4`.  The three-star partition has upper energy
`4*31+6*76+111=691<721+26C`.

For `(2,1,1)`, the broad upper 725 against `693+26C` gives `C<=1`.  Its
`P=7` direction uses six displacement classes, so independently `C>=1`.
At equality the only duplicate lies in that direction's zero bin and every
transverse multiplicity is Boolean.  The live `F_13^2` bucket audit gives six
...
it left `u in {4,6}` at that stage. Proposition 15.749 later removes `u=4`,
and Proposition 15.754 later removes `u=6`.

## 15.744 — six-root quartic closes `p=13,t=4,u=3`

The full `t=4` floor/lift sieve leaves `u in {0,3,4,6}`.  In `u=3`, the
hard quotient profile is six exact complement triples plus one elevation.
The exact-row coefficient congruence and edge count leave `P=2,8`; the two
parallel ledgers force an opposite mass-14 cell at `Q=6` or `Q=0`.

The phase-one `b=10` premise is not inferred from its averaged floor value.
After complementing to phase-zero `b=3`, positive quadrature forces equality
on the three contact layers, and their 1,596-by-78 pair-monomial matrix has
rank 78 modulo 101, so the exact cell is pointwise `(2-r)^2`.  At mean 22
the difference from this baseline can be `-2` on `r=0`, so Proposition
15.688 does not apply.  The separate 1,716-variable punctured-lift model,
using all 1,638 third-difference identities and `sum B=66`, is infeasible.
This kills the `u=4,b=10` alternative and leaves only the sharp `b=2`
mass-ten lift there.

Proposition 15.738's Boolean support classification is edge-count
independent, but its height-four preliminary models used `|H|=59`.
Proposition 15.744 rebuilds both models at `|H|=61`, with `l1<=61-Q`; exact
one-worker CP-SAT makes both infeasible.  The Boolean catalog then leaves
only `B=x_i*x_j`.  The six exact hard directions are roots of the binary
quartic `G=2hM_4-M_2^2`, so it vanishes identically, while the forced
opposite selected pair gives `G=-3(i-j)^4!=0`.  This closes exactly `u=3`,
not the full `p=13,k=60` row.

Proposition 15.746 completes the mass-ten classification and records the
branchwise mean-12 consequences; its section above is the current finite
gate.

## 15.743 — full translated-cut energy closes `p=17,k=74`

At `p=17,t=3`, the nine hard quotients have excess partition `(3)`,
`(2,1)`, or `(1,1,1)`, leaving eight, seven, or six exact stars.  Hence the
global difference moments `M_2` and `M_4` vanish.  The signed normalization
is not assumed from the specialized star row.  For an exact row `k=1`, the
unspecialized local/common equality is `17P-69=hT-P`, hence
`hT=18P-69`; all exact stars therefore share `P`.  Six such directions give
`P<=12`, and the literal congruence `P=5 mod 8` forces `P=5`, after which
`hT=21` and `q=(2)^8`.  This agrees independently with the hard/opposite
edge totals `48/27`.

For a general hard quotient `k`, equating the local sum `17(P-3)-18k`
with the common-graph sum `hT-P=21-P` forces
`P=4+k`.  This is the cross-direction normalization; local rows with other
`P` do not come from the common graph.

The 24,310 nine-sets give exactly 698 distinct translation-summed cut
vectors on the eight cyclic distances.  Broad-domain one-worker CP-SAT
models impose only the exact sum, `l1`, `M_2=M_4=0 (mod 17)`, and those 698
upper cuts.  They prove that excess `e=1` is infeasible, while the sharp
energies for `e=2`, `e=3`, and opposite rows are `70`, `119`, and `72`.
No prior energy cap, entry alphabet, lower cut, or unverified full-row count
is used.

The two partitions containing `e=1` are empty.  For the sole remaining
`e=3` partition, one elevated and nine opposite rows have energy at most

`119+9*72=767`,

whereas the exact p17 difference-Radon identity requires `1211+34C>=1211`.
Thus `p=17,k=74` is empty. Proposition 15.751 later closes the generic
branch-B `t=3` range and Proposition 15.752 closes its displayed higher
band. This finite certificate alone closes neither; residual (ii) and the
limit remain open, while multi-level Type I is later closed by 15.750.

## 15.742 — six-dilate energy closes `p=13,k=58`

For each of the ten nonexact six-bin rows from 15.741, retain the integral
sum, the global `M_2=0` congruence, and the six translated-cut inequalities
coming from the multiplicative dilates of `{0,...,6}`.  Exact bounded
enumeration leaves 30 elevated rows, of sharp energy at most 31, and 24
opposite rows, of sharp energy at most 82.  Independent 19-variable CP-SAT
models exclude energy at least 32 and 83, and an independent slack-coordinate
enumerator gives the same row hashes and maxima.

The three elevated and seven opposite rows would therefore have total energy
at most

`3*31+7*82=667`.

This contradicts 15.741's exact common-graph Parseval identity
`707+26C>=707`.  Thus the generic four-exact `p=13,t=3` partition is empty.
Together with 15.739's exceptional close, all of `p=13,k=58` is closed.
...
exhaustive finite certificate, not a global residual-(ii) close. At this
15.742 stage, critical `p=5,7`, `p=11,k>=50`, `p=13,k=60,u=6`, later p13
layers, and the p17/p19 fifth-shell endpoints remained; 15.753 later closes
the p17/p19 endpoints and 15.754 later closes the p13 `u=6` endpoint, while
layers beyond 15.752's later band remain;
branch-B `t=3` is later closed by 15.751; 15.743 closes `p=17,k=74`,
15.744--15.745 close `u=3,0` at
`p=13,k=60`, and 15.746 gives the support-330/mean-12 open reduction for
`u=4`.

## 15.741 — common-graph moment and difference-Radon gate

For the last generic partition `1^4 2^3`, retain the single 59-edge graph.
Four exact positive-star projections force the global homogeneous forms
`M_2=T_3=0`.  The two orientation-independent quartics satisfy
`U_4=lambda M_4`, with `M_4!=0`: if `M_4` vanished, Proposition 15.740's
opposite-cell certificate would already give a contradiction.  This
low-degree consequence is sharp locally.  One elevated cell and one opposite
cell meet every one-direction sum, `l1`, parity, cut, and moment condition
with the same affine-label-invariant ratio `lambda=7`.  They are not a common
graph.

Let `m_delta` count edges in the 84 classes
`(F_13^2\{0})/{+-1}` and let `q_L(a)` be the six signed cyclic-distance
aggregates in direction `L`.  The 98-by-84 incidence transform satisfies

`B^T B=13I+2J-G`

and has the exact integer inverse recorded in Proposition 15.741.  With
`P=5^4,6^3,3^7`, the four exact rows are `(2,2,2,2,2,2)` and

`sum_nonstar ||q_L||^2 = 707+26C`,

where `C=sum_delta binom(m_delta,2)`.  Six multiplicative translates of one
balanced cut give only `C<=11`.  A strict rational interior witness proves
that the bare direction-count/exact-row/translated-cut relaxation cannot be
closed by linear/Farkas averaging; it does not test the nonzero quartic or
midpoint identities.  At the time this left a layered 84-class inverse and
binary-midpoint gate.  Proposition 15.742 closes earlier: the integral
`M_2=0` congruence plus the same six cuts force sharp row energies whose
total is already below this Parseval identity.

## 15.740 — translation averages leave one generic `p=13` partition

The generic p13 hard quotient ledger has exactly three partitions:
`1^6 4`, `1^5 2 3`, and `1^4 2^3`.  Five exact hard stars force the global
binary forms `M_2` and `M_4` to vanish.  For any opposite `Q=3,b=0` cell,
pair-conditioned cuts give `W_ij>=-1`.  Aggregating its coefficients over
the six cyclic distance classes produces integers `n_a` in `[-13,18]` with
sum `-20`, `l1<=56`, and vanishing quadratic and quartic moments modulo 13.
Summing each cut inequality over all 13 translations gives 74 possible
linear inequalities in those six variables.

Exact enumeration leaves 32,313 rows before translated cuts.  Nine
deterministically selected inequalities eliminate all of them; an
independently encoded 14-variable CP-SAT model is also infeasible.  Hence
the five- and six-exact partitions are impossible.  Only `1^4 2^3` remains.
This was the stopping point for independent local coefficient catalogs.  The
remaining structural common graph realizes four exact `P=5` stars, three
elevated `P=6` hard cells, and seven `Q=3,b=0` opposite cells; 15.741
compresses it and 15.742 excludes it.  Proposition 15.692 supplies the binary affine-Radon
reconstruction; do not renumber or rerun it.

## 15.739 — exceptional `p=13,t=3,u=3` row closed; generic moment frontier

The corrected complement-triple target has coefficient offset two.  Thus
the seven exceptional hard directions have common parallel count `P=2` or
`P=8`.  Exact opposite edge accounting forces at least four phase-zero
mass-14 cells at `Q=6` or `Q=0`, respectively.  Proposition 15.738 reduces
either cell to `B=x_i*x_j`.

For even `d`, retain the global homogeneous form

`M_d(L)=sum_({u,v} in H) chi(u-v)(L(u)-L(v))^d`.

If `h` is the common hard sign, both exceptional hard gauges satisfy
`2S_4=S_2^2`.  Hence `G=2hM_4-M_2^2` is a binary quartic with seven
projective roots and is identically zero.  An opposite selected-pair cell
instead has `S_2=(i-j)^2`, `S_4=(i-j)^4`, and therefore
`G=-3(i-j)^4!=0` in `F_13`.  This contradiction closes exactly the
exceptional row.

For the generic branch at `p=1 mod 4`, `p>=17`, at least `(p-5)/2` exact
hard stars force every even moment through degree `(p-9)/2` to vanish.
Conditional balanced-cut averages force the opposite signed matrix alphabet
to `{-1,0,1,2,3}`.  At `p=17` this yields an exact bounded integer model
with all 24,310 middle cuts and moments `M_2=M_4=0`.  A two-intersection
stabilizer average further puts every cut in `[-26,-12]`, so its associated
quadratic has values `{0,...,7}`, mean `6/17`, and total mass 8,580.  Current
complete-domain runs were `UNKNOWN`, not certificates; Proposition 15.743
later bypasses them with the full 698-vector translated-cut/common-energy
certificate and closes `p=17,k=74`.  At generic `p=13`, an explicit local
elevated cell has normalized `S_2=0` but `S_4=5` (so global `M_4=5h`),
proving that another independent
floor/moment pass is insufficient.  It is a method counterexample, not a
residual graph.

## 15.738 — exact `J(13,7)` mass-14 Boolean catalog

For the phase-zero `b=0,a=14` cells forced by the exceptional ledger,
Proposition 15.688 leaves height one or four.  Exact coefficient, cut,
row-parity, and `l1` models at `Q=0` and `Q=6` exclude height four.  On
`J(13,7)`, the 78 pair evaluations have rank 78 and 1,638 independent
third-difference identities span their annihilator.  An anchored exact
CP-SAT certificate proves that all support-462 Boolean quadratics are the
78 selected pairs, 156 oriented mixed pairs, and 858 signed mixed triples.
Their coefficient offsets are `6,4,4`, so only `x_i*x_j` survives modulo
six at `Q=0,6`.  Its exact moments are `(i-j)^2` and `(i-j)^4`.  This is an
exhaustive finite cell certificate and the local input to 15.739; by itself
it is not a residual theorem.

## 15.737 — binary quadratic moment closes `p=11` at `k=44,46,48`

For layer index `t=0,1,2`, put `k=44+2t` and `|H|=45+2t`. The support bound
leaves respectively `32,28,24` isolated projective vertices, so signed PSL
transport gives `I=0`, an all-finite boundary, and even directional `b_d`.
The exact p11 phase-one ledger is

`sum a_d=12(6+t)`, `a_d=2u+12k_d`, `sum k_d=6+t-u`.

Residues `0<=u<=t` force a nonzero lift of excess at most six, below the
sharp floor eight; residues `t<u<5` have too few quotient units. At `u=5`,
at least `5-t` exact mean-ten hard directions remain. Positive quadrature
fixes their baselines to `b=2` (offset 4) or `b=10` (offset 3), and the two
offsets cannot mix.

Proposition 15.736's catalog excludes the all-`b=2` branch. In the `b=10`
branch define, for an `F_11`-linear fibre functional `L`,

`M_H(L)=sum_({u,v} in H) chi(u-v)(L(u)-L(v))^2`.

This is a homogeneous binary quadratic in `L`. Each hard literal baseline
has signed cell matrix a star, so
`M_H(L)=-eps_L sum_(s!=j)(s-j)^2=0`. The at least `5-t>=3` hard directions
therefore force `M_H` identically zero. But each surviving opposite
all-equal triple has moment
`(i-j)^2+(i-k)^2+(j-k)^2`. Normalizing its distinct labels to `(0,1,r)`
gives `2(r^2-r+1)`, which never vanishes because its discriminant
`-3=8` is nonsquare modulo 11. The exact edge ledger forces at least
`4-t>=2` such triples, although one already contradicts `M_H=0`.

Thus `p=11`, `k=44,46,48` is CLOSED for every boundary size. The argument
stops exactly at `t=3`: only two hard stars remain and excess eight reaches
the equality floor.

## 15.736 — exact sharp Boolean-quadratic catalog at `p=11`

On `J(11,6)`, the 55 pair-monomial columns have exact real rank 55. A
deterministic set of 407 independent third-difference identities spans their
annihilator. CP-SAT then proves that the 55 omitted-pair and 165
all-equal-triple supports exhaust all Boolean quadratic supports of sharp
size 84. This is an **exhaustive finite certificate**, not an all-prime
classification theorem. It replaces the former conditional external
restriction-extension route. Proposition 15.688 supplies the Boolean
equality rigidity; the catalog offsets 2 and 4 are the finite input used by
15.737.

## 15.735 — the next two layers close for every `p>=13`

The isolated-chart proof persists at `k=4p+2t`, `t=1,2`: the exact type
budget is `2m(m+t)`, the phase-one type alternates with `t`, and all low
residues except the same three exact baselines are lifts of excess at most
six. Coefficient offsets again force `s=4,5,3`; opposite counting again
produces scaled mean `8,6,8`, below the nonzero-`b` and integral-lift floors
for `p>=13`. Hence `k=4p+2,4p+4` is empty for every boundary size and every
prime `p>=13`.

Together, 15.734--15.735 and 15.737 close
`k in {4p,4p+2,4p+4}` for every prime `p>=11`. Proposition 15.751 later
closes the fourth shell and 15.752 closes the fifth shell for `p>=23` plus
its stated higher band. At this 15.735 stage, the remaining residual scope
included critical `p=5,7`, `p=11` at `k>=50`, `p=13,k=60,u=6`, later p13
layers, and the small fifth-shell endpoints; 15.753 later closes the p17/p19
endpoints and 15.754 later closes p13 `u=6`, while layers beyond 15.752's
band remain. Proposition 15.742 closes the
intervening `p=13,k=58` row,
...
a seven-root quartic contradiction. Residual (ii) globally and `L` remain
OPEN; multi-level Type I is later closed by 15.750. The next finite p13
residue at this stage was `u=6`; Proposition 15.754 later closes it. Do not
reopen the closed `u=0,3,4,6` rows or the support-330 catalog.

## 15.734 — every critical `k=4p` boundary is excluded for `p>=13`

Let a critical residual-(ii) witness have `|H|=4p+1`. At most `8p+2`
projective vertices are incident with its edges. Since
`p^2+1>8p+2` for `p>=13`, choose an isolated vertex and use Proposition
15.721's signed PSL transport to send it to infinity. The transported chart
has `I=0`, `infinity notin D`, and every directional `b_d` even.

Put `q=(p-1)/2` and `m=q+1`. The exact type budget and common-residue
identity, the phase floors, Proposition 15.652's positive-quadrature equality
baselines, and Proposition 15.688's two-unit lift obstruction leave three
hard-type branches with coefficient offsets `4,5,3`. With `I=0` these force
`s=4,5,3`, and opposite counting forces a phase-zero direction of scaled
mean `8,6,8`. Both the nonzero-`b` floor and the `b=0` integral-lift floor
exceed those means for `p>=13`.

This is a proved theorem for every boundary size, not a continuation of the
old `|D|=p+1` shell campaign. Later Propositions 15.735 and 15.737 extend the
first-three-layer close stated above. Historical text below that calls the
`p=31` endpoint, critical `p=11`, or any first-three `p>=11` layer open is
superseded by 15.733--15.737.

## 15.733 — simultaneous baselines close the historical `p=31` endpoint

For `p=31`, `|H|=125`, `|D|=32`, and `R=10`, all fifteen hard mean-30
directions are exact `b=2` baselines: the alternative `b=30` baseline has a
coefficient offset differing by one modulo 15 and cannot mix. Put
`rho=(I+P-4)/15` and `s=rho+P`. Opposite-type mass and the sharp integral
lift floor force `s=8`; hence every opposite direction has mean 32. Its
floor-compatible nonzero baselines would require `15|(I-4)` or
`15|(I-5)`, incompatible with `I=124-16P`, `1<=P<=7`. Thus all sixteen
opposite directions have `b=0`, and `sum_d b_d=72` would force the remaining
hard direction to have `b=42>30`. This symbolic contradiction closes the
endpoint without a finite configuration census. Proposition 15.734 later
subsumes it.

## 15.732 — exact repair cycles and the actual phase-bridge target

The linear cycle proposal at the end of 15.731 is now closed as a method.
For every repair define

`P_A=product_(u in A)L_u`, `Theta_A=P_A^2 Phi_A`.

On an oriented adjacent swap `A=C union {a}` to `A'=C union {z}`, the edge
law becomes

`Theta_A'-Theta_A=P_C^3 Q_(a,z)`.

Thus every cleared transition is an explicit vertex-potential difference,
and every closed-walk sum vanishes identically. Evaluation, differentiation,
polarization, and coefficient extraction cannot make that circulation
nonzero. In `p=3R+1`, changing `Phi_A` by `mu_A P_A` adds `mu_A P_A^3` to
the vertex potential, so the cubic-pencil gauge remains a coboundary.

On a three-point block the exact triangle relation is

`L_i^3 Q_(j,k)^i+L_k^3 Q_(i,j)^k+L_j^3 Q_(k,i)^j=0`.

The same relation occurs on every triangular face of `J(4,2)`; its square
cycles telescope as well. These are consistency identities, not holonomy.

There is a genuine edge-local invariant. At the dual point `q` of the
swapped rich line, `Q(q)=0` and `j_q^1 Q` is nonzero. It is unchanged by the
cubic lift gauge, and its square character is the repair-coloured factor
`chi(P_B(q))`. Residual parity determines the symmetric difference of the
`A`- and `T`-tangent fibres, not that individual colour. Products over
repairs cannot generally repair the loss: the three pair masks on a
trisecant span the even-weight subspace and miss `111`.

The 15.728 direction-component route is also unavailable. A nonrich `b=2`
near-pairing direction leaves at least `(p-1)/2-R` repair secants and hence
at most `R+2` distinct known repair-tangent roots on the dual direction line,
against envelope degree `2R+2`. At `p=31` this is 12 roots versus degree 22 in each
of at least `4+y` nonrich Paley-hard directions.

This is a **proved method barrier, not endpoint closure**. Do not launch
another bare cycle evaluation or tangent-component count. The closest live
front is the simultaneous coefficient system supplied at `p=31` by at least
fourteen same-Paley exact baselines `A_d=(1-x_s-x_t)^2`. A successful
envelope bridge would instead have to express a lift-independent mixed
polarization in a nonrich hard direction in terms of those signed
coefficients. Neither statement is presently proved.

## 15.731 — tangent-envelope gluing and low-degree repair swaps

For any 15.730 repair `A`, put `t=R+1` and `d=2t=2R+2`. Square the
Segre-normalized tangent products and regard them as degree-`d` sections on
the dual lines `a*`, `a in A`. Segre's tangent lemma makes the sections agree
at every pairwise node. A direct line-gluing argument therefore gives a
degree-`d` tangent-envelope polynomial `Phi_A`.

| residue | `|A|` | normalized envelope space |
|---|---:|---|
| `p=3R+1` | `d` | `Phi_0+lambda product_(a in A)L_a` |
| `p=3R+2` | `d+1` | unique |

This is a consequence of the Segre tangent lemma below the sufficient size
hypothesis in Ball--Lavrauw's explicit interpolation theorem; it is not an
application of that theorem outside its assumptions. No tangent-envelope
existence conditions are missing at the endpoint.

Adjacent repairs have a much smaller transition object. If
`A=C union {a}` and `A'=C union {z}`, their Segre normalizations can be
aligned so that

`L_z^2 Phi_(A')-L_a^2 Phi_A=P_C Q_(a,z)`.

The quotient is quadratic for `p=3R+2`. For `p=3R+1` it is cubic, defined
modulo `span{L_a^3,L_z^3}` because of the two envelope pencils. Every repair
shares at least two singleton points, providing an anchor for coherent
normalization before edge identities are composed around a cycle.

This is a **proved algebraic refinement, not endpoint closure**. The repair
graph is a product of `K_3` and `J(4,2)` factors. The exact open implication
is exclusion of the common completion under 15.728/15.730's residual
direction, phase, and lift constraints. Proposition 15.732 proves that every
naturally cleared linear cycle sum is an exact coboundary and replaces that
proposal by the rich-direction first-jet / simultaneous-baseline fronts.
Endpoint equality, larger slack, residual (ii), Type
I, and `L` remain OPEN.

## 15.730 — complementary-arc repair ensemble and exact census

Fix any surviving 15.727 endpoint decomposition and write
`D=A_Q disjoint-union T_Q`, `k=|A_Q|=p+1-R`. Every maximum arc contained in
`D` is obtained by retaining every singleton point and choosing exactly two
points on each rich block. Therefore there are exactly `3^x 6^y` maximum
repairs. For every repair, `T_Q` is itself an `R`-arc, every `z in T_Q` has
exactly one `A_Q`-secant, and all `R` one-point extensions
`A_Q union {z}` are affine `(k+1,3)`-arcs with a unique trisecant.

The full projective two-colour census is
...
On a rich block containing the repair pair `{a,b}`, the base
`A_Q\{a}` has `{a} union (T_Q intersect L)` as pairwise-incompatible
extensions on the same tangent through `b`. Thus each fixed repair gives
`2x` bases with two co-tangent extensions and `2y` bases with **three**.

Direction-wise, if `sigma_d,tau_d` count the two colours' secants and `m_d`
counts `(1,1)` lines, then

`b_d=p+1-2(sigma_d+tau_d+m_d)`.

The nine component fibre counts are explicit in Proposition 15.730. This is
a **proved structural reduction, not endpoint closure**. It supplies a
coupled product family, rather than one selected unique-trisecant set, for
the next tangent or phase-specific attack.

The former statement that Bartoli--Storme gives an odd-order
unique-trisecant size ceiling is **RETRACTED**. Under their other hypotheses,
including `d>3+2sqrt(q)` and existence of the configuration, their threshold
is the upper endpoint of a range in which the associated hyperplane
arrangement is second-smallest; it is not a nonexistence theorem for larger
3-arcs. Every repair arc is two
points (`p=3R+1`) or one point (`p=3R+2`) below the sufficient size
hypothesis of Ball--Lavrauw's explicit formula, but 15.731 constructs and
classifies its tangent envelopes directly. Proposition 15.732 then proves
that their bare linear cycle circulation is exact. The exact open implication
is exclusion of the common completion under a genuinely new signed bridge,
or directly under the direction equations and residual phase/lift constraints.
Endpoint equality, larger slack, residual (ii), Type I, and `L` remain OPEN.
...
affine unique-trisecant sets, equivalently the co-tangent extension arcs,
while retaining compatibility with their common disjoint-block completion
`D`. Propositions 15.730--15.732 supersede that formulation with the full
repair ensemble, its transition coordinates, and the exact-cycle barrier.
After removing one trisecant point, the arc `A` has size `2R+2` or `2R+3`,
two or one points below the sufficient hypothesis `2R+4` in
Ball--Lavrauw's explicit tangent interpolation. Proposition 15.730 replaces
the former misread Bartoli--Storme ceiling route with the exact coupled
family of all repairs, 15.731 constructs their tangent envelopes directly,
and 15.732 identifies the surviving gauge-invariant local jet.
...
32-point sets. Propositions 15.731--15.732 are the latest all-prime
algebraic refinements; the `p=31` endpoint remains open.
...
15.728--15.732. None of these reductions excludes it. Larger slack, the rest
...
92,664 directions for primes 17 through 47), but it is not an all-prime or
two-orientation theorem. The asserted Hasse bounds omit their discriminant
and resolvent curve proofs, including the admissible singular locus
`4*a*nu+1=0`, and the opposite product sign is unchecked. Do not use 15.725
downstream.

## 15.722--15.724 — exact phase transport and full-circle closure

At the first general boundary `|D|=p+1`, 15.722 tracks the signed PSL phase
instead of treating chart signs independently:

`c_(gH)=c_H product_(v in boundary(H)) delta_g(v)`.

For `g(z)=(az+b)/(cz+d)`, the pole/infinity value `chi(c)` applies only
when `c!=0`.  In the affine case `c=0` there is no pole and the multiplier
at finite points and infinity is `chi(d)=chi(a)`.  This corrected case split
does not alter the inversion charts used below.

For an all-finite boundary with root polynomial `f`, boundary-point and
outside-point charts have signs `c_v=c_H chi(f'(v))` and
`c_w=c_H chi(f(w))`. Outside pair slack one would be a unique trisecant and
is impossible. Pair slack zero is a `p+1`-arc; Segre plus a genus-one
character bound forces it to be a Miquelian circle with exact direction-type
alignment. One-point Miquelian-circle replacements are already excluded by
a second chart and 15.676.

For a full circle normalized as `{infinity} union (a+b F_p)`, 15.722 proves
`c_H=(-1)^m`, so every circle-point chart has the easy phase `m mod 2`.
Proposition 15.724 then closes the branch. Because `H` has `4p+1` edges,
there is an isolated outside vertex. Sending it to infinity gives `I=0`,
`m` phase-zero `b=0` directions and `m` phase-one `b=2` directions. The
phase-one xnor type has `m-1` baselines and one elevation. Exact finite-edge
counting and its baseline congruence force `(u,x,y)=(4,4,3)`. At least four
phase-zero directions then have `A=2B` with `4p E[B]=8`, contradicting
15.688's `4p E[B]>=p-3>=14`. Hence outside pair slack zero is empty for
...
`(p-1)c=I+P_d-4`.  Do not describe both older baselines as identical XNOR.

Proposition 15.723 separately repairs every use of the middle
floor-plus-two shortcut. Its paired-cube theorem excludes all such odd
profile cells for `p>=17` except the real `p=17` cells `(b,phase)=(5,1)`
and `(11,0)`. Both have explicit `A=(t-3)^2` witnesses. Any profile code
must retain those two cells. Do not restore a blanket `excess != 2` rule.
...
The Hilbert basis from 15.718 has maximum generator grade three.  In any
finite projected group, equality of four consecutive recurrence layers
therefore makes the common support the subgroup generated by all projected
Hilbert generators and proves permanent uncapped stabilization.  The complete
`k=3` and `k=4` certificates have identical raw and anchor-relative supports
at grades 3, 4, 5, and 6 in every direction.  The subgroup orders are
...
All Hilbert generators are binary, so these projected catalogs are cap-exact
through grade six.  Grade eight is only an outer support: projected presence
cannot certify the discarded coordinate cap `L_X<=6` or a binary edge lift.

Strategy context only: exact `H0_S0_M7` torsion projections from `k=2`
through `k=6` saturated or otherwise retained every target and are closed as
nonseparating.  The bounded grade-three-only case-join runs processed all 51
...
## 15.718 — global z7 reduction and Johnson-semigroup certificate

The 56 line boundaries in the two positive `z=7` source orbits produce 4,320
pointed branch cases.  The exact parent affine-hull sieve rejects 3,024 and
leaves 1,296.  Exact affine symmetry partitions all survivors into 324
four-case classes.  A same-catalog-row mod-3/mod-7 global join processes 246
representatives and gives

| representative decision | representatives | transferred pointed cases |
|---|---:|---:|
| rigorous empty join | 87 | 348 |
| necessary-only survivor | 159 | 636 |
| explicit budget skip | 78 | 312 |
| **total** | **324** | **1,296** |

The 348 transferred rejections are pointed leaf cases, not source line
boundaries, so the actual boundary count does not decrease.

Independently, Normaliz gives a complete 896-row binary Hilbert basis for the
Johnson incidence semigroup: 56 generators in grade one, 168 new generators
in grade two, and 672 new generators in grade three.  The exact uncapped
layer census is

| grade | 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| rows | 1 | 56 | 1,764 | 37,856 | 575,407 | 6,496,938 | 57,232,105 | 410,200,367 | 2,474,264,653 |

The coordinate cap is automatic through grade six and must be enforced
explicitly at grade eight.  This certificate closes the structure of the
high catalogs, not case or binary-edge feasibility.  All 56 `z=7`
boundaries, their two orbits and two projected profiles, the positive
endpoint, and the theorem remain open.
...
869 -> 321 -> 19 -> 14 -> 0`. Proposition 15.705 is partial and its 74
uncovered slack-sixteen rows are removed only at 15.709.
...
Every one of the 321 rows left by 15.709 has nine rigid phase-one `b=16`
directions. Of these, 270 retain a genuine floor-rigid phase-zero `b=0`
direction. Their
...
and gauge sum 16, but the gauge minima are 9 and 8. All 302 are impossible.
...
The rigid-anchor arguments of 15.708 do not depend on pair slack. Among the
869-profile ledger, exactly 548 rows have `u_1=8`, and every one retains at
least eight rigid phase-one `b=2` directions. The 334 `(0,8)` rows retain
between two and five rigid phase-zero `b=0` directions, so the global gauge
contradiction applies. The 214 `(8,8)` rows retain between two and eight rigid
phase-zero `b=16` directions, so the unique-even-fibre contradiction applies.

The first block contains all 74 slack-sixteen rows left outside 15.705's
historical Orbiter target set. The p17 ledger drops from 869 to 321 profiles.
Every survivor has `u_1=0`, with residue split `(0,0):275`, `(7,0):9`,
`(8,0):37`; the minimum remaining pair slack is 96. No solver or new
classification is used. The endpoint and all top-level gates remain open.

## 15.708 — the entire p17 slack-twenty-four block is excluded

For the 142 `(u_0,u_1)=(0,8)` rows, quotient accounting retains between two
and five rigid phase-zero `b=0` directions and at least eight rigid phase-one
...
All 151 slack-twenty-four profiles are excluded analytically. The live p17
ledger is 869. No solver or new classification is used; the endpoint and all
top-level gates remain open.

## 15.707 — the entire p17 slack-twenty block is excluded

For all 184 `(u_0,u_1)=(0,8)` rows, exact quotient accounting retains at
...
complete-14-minus-one core at most four. Thus all 193 slack-twenty profiles
are impossible. The live p17 ledger is 1,020. No new classification or
...
or new classification input is used. The corrected prefix is
`2503 -> 2219 -> 1744 -> 1481 -> 1368 -> 1228 -> 1215`: 15.700--15.704
exclude `284,475,263,113,140`, while 15.705 is **PARTIAL/OPEN**, excludes
only its thirteen historical targets, and retains 74 slack-sixteen rows.
This 15.706 step gives `1215 -> 1213`; those 74 rows survive here and are
removed only by 15.709. The p17 endpoint remains open at this stage.
...
All 68 rows are impossible inside 15.684's conditional residue-zero
subledger. Exactly 133 profiles remain there, all of slack at least 24. The
restored `u_0=9` branch lies outside this count, so the endpoint and all
top-level gates remain open.

## 15.686 — unique `p=23` slack-16 profile excluded

`src/e1_gmin_m4_prop15686.py` and
`evidence/NOTE_2026-08-28_p23_slack16_complete17_exclusion.md`. This row
has one undetermined direction. If repair deletes at most three points,
adjoining its infinity point gives an arc of size at least 18 and the
conic-core contradiction. In the only hard branch, four deletions leave a
16-arc `A`; adjoining the infinity point gives a 17-arc `K`, which must
be complete or it again extends to an 18-point conic core.

For every deleted point `d`, the undetermined direction ensures that no
secant of `K` through `d` uses the added infinity point. Completeness and
the slack inequality therefore force all four deleted points to have
secant multiplicity one outside `K`. Proposition 15.685's exhaustive five
complete-17-arc representatives show that every class has at most one.

The unique slack-16 profile in the conditional residue-zero subledger is
impossible. Exactly 201 profiles remain there, all of slack at least 20. The
restored `u_0=9` branch is unaffected, so the endpoint and all top-level gates
remain open.
...
The unique slack-12 profile in the conditional residue-zero subledger is
impossible. Exactly 202 profiles remain there; the restored `u_0=9` branch,
this endpoint, `p=17,19`, later sizes, residual (ii), R1, QVAR, Type I, and
the limit remain open.

## 15.684 — `OPEN_RETRACTED_REDUCTION` at `p=23,s=20`

`src/e1_gmin_m4_prop15684.py` and the historical note
`evidence/NOTE_2026-08-28_p23_low_mass_conic_reduction.md`. The corrected
floor-plus-two classifier restores phase-zero `u_0=9`, with scaled mass 18
and an explicit slack-zero profile. Thus the old claim that only residue zero
remains is false.

The old residue-zero computation is still a valid conditional subledger:
completion-bounded enumeration gives 1,247 phase-labelled `u_0=0` profiles.
Segre's degree-10 tangent envelope and the complete-arc/conic-core arguments
exclude 1,044 and leave 203 within that subledger, including the two
low-slack exceptions and every row of slack at least 20.

This does not reduce the restored `u_0=9` branch, so 15.684 is a retracted
reduction rather than a theorem. Proposition 15.721 independently excludes
the all-finite boundary as a live gate by signed transport. Strict
infinity-plus-`p`, residual (ii), R1, QVAR, Type I, and the limit remain open.

## 15.683 — `p=41,s=34` next all-finite endpoint closed

`src/e1_gmin_m4_prop15683.py` and
`evidence/NOTE_2026-08-28_p41_next_all_finite_endpoint.md`. Proposition
15.681 removes every positive residue. Exact residue-zero enumeration leaves
seven 34-arc profiles and two profiles with one 3-secant. All have fourteen
perfect phase-zero directions, twenty phase-one directions missing two
points, and eight exceptional directions.

For an arc row, the exceptional directions contain only three secant edges
and at least 28 tangents each. Ball--Lavrauw Theorem 11 gives Segre's
degree-18 tangent envelope with square restrictions on every point pencil.
The eight exceptional direction-pencil lines divide it twice, leaving a
conic. The three exceptional edges touch at least three arc points, each
with two remaining tangents, forcing three point-pencil lines into that
conic.

For a one-triple row, delete a triple point chosen to preserve the unique
exceptional floor-secant as a pair. The resulting 33-arc has exceptional
tangent counts `33^7,31`. Their squares leave a quartic from the degree-20
envelope. The surviving pair endpoints force two point-pencil factors;
three of the other 31 points then force three lines into the residual conic.
Both cases contradict degree.

Thus `p=41,s=34` is impossible. The same boundary remains open only at
`p=17,19,23`. Later all-finite sizes, strict infinity-plus-`p`, residual
(ii), R1, QVAR, Type I, and the limit remain open.

## 15.682 — `p=31,s=26` next all-finite endpoint closed

`src/e1_gmin_m4_prop15682.py` and
`evidence/NOTE_2026-08-28_p31_next_all_finite_endpoint.md`. Proposition
15.681's scaled lift floor 16 removes every positive phase-zero residue.
The exact residue-zero enumeration leaves fourteen phase-labelled profiles:
eleven 26-arcs with at least three undetermined directions, and three sets
with exactly one 3-secant and at least five. Deleting one point of the
unique triple gives a 25-arc.

Coolsaet's exhaustive classification has no complete arc in `PG(2,31)` of
size 23 through 31. Every 27-/28-arc therefore extends to a complete
32-arc, which is a conic by Segre. Adjoining pairs from three undetermined
infinity points to either the 26-arc or deleted 25-arc forces two conics to
share at least 25 affine points and hence coincide. That conic would contain
three collinear infinity points, impossible.

Thus `p=31,s=26` is impossible. Proposition 15.683 subsequently closes
`p=41`; the same boundary remains open at `p=17,19,23`. Later all-finite
sizes, strict infinity-plus-`p`, residual (ii), R1, QVAR, Type I, and the
limit remain open.

## 15.681 — integral paired-cube lift and `p=29,s=24` closed

`src/e1_gmin_m4_prop15681.py` and
`evidence/NOTE_2026-08-28_p29_next_all_finite_endpoint.md`. For a nonzero
nonnegative integral quadratic `B`, applying the paired-cube operator at a
point with value `h` gives `4p E[B]>=p+1-4h`. Combining this with 15.642's
stabilizer inequality gives the all-prime scaled floor `(p+1)/2` for
`p=3 mod 4` and `(p-1)/2` for `p=1 mod 4`.

The floors `14,16,18,20` kill all positive endpoint residues at
`p=29,31,37,41`. At `p=29`, the exact residue-zero ledger leaves four
phase-labelled 24-arc profiles with at least four undetermined directions
and one one-triple profile with six. Deleting one triple point in the latter
case gives a 23-arc. Coolsaet--Sticker classify all 25-/26-arcs in
`PG(2,29)` into 10/5 classes; exact `PGL(2,29)` enumeration gives exactly
10/5 conic-complement orbits, so all are conic-contained. Extensions through
three undetermined infinity points force one conic through three collinear
points, a contradiction.

Thus `p=29,s=24` is impossible. Propositions 15.682--15.683 subsequently
close `p=31,41`. The same boundary remains open at `p=17,19,23`. Later
all-finite sizes, strict infinity-plus-`p`, residual (ii), R1, QVAR, Type I,
...
Thus `p=37,s=30` is impossible. Propositions 15.681--15.683 subsequently
close `p=29,31,41`; the same boundary at `p=17,19,23`, later all-finite
sizes, strict infinity-plus-`p`,
...
Propositions 15.680--15.683 separately close `p=37,29,31,41`; the smaller
endpoints `17,19,23`, later all-finite sizes, strict infinity-plus-`p`,
residual (ii), R1, QVAR, Type I, and the limit remain open.

## 15.678 — `OPEN_RETRACTED_REDUCTION` at the `p=17` first survivor

`src/e1_gmin_m4_prop15678.py` and the historical note
`evidence/NOTE_2026-08-28_p17_first_all_finite_survivor_exclusion.md`. The
corrected census has 108 compatible profiles spanning 47 arc profiles, not
two pair-equality arcs. The retained lift/coefficient lemmas remain valid,
but they do not reduce the endpoint to the two profiles used by the old
geometry.

The unique-16-arc/conic argument still excludes 14 corrected arc profiles,
but 94 compatible profiles are uncovered. Therefore the historical
15.675--15.678 chain does not close `p=17`. Proposition 15.721 later
supersedes this all-finite boundary gate and excludes it by signed transport.
...
15.678's attempted close is retracted, and 15.721 later supersedes the whole
all-finite gate by signed transport. Proposition 15.679
closes the next size from `p=43`, 15.680 closes its `p=37` endpoint, 15.681
closes `p=29`, 15.682 closes `p=31`, and 15.683 closes `p=41`; three smaller
endpoints, subsequent sizes, strict deficit in infinity-plus-`p`, residual
(ii), R1, QVAR, Type I, and the limit remain open.
...
`scripts/maxplus_profile_enum/` now lives on main. That historical enum campaign
is still not a leftover close. Historical worker names, stop flags, and
process state are deliberately not proof context. Worktree
...
Leftover 3 has two independent sufficient targets, neither imported. p=5 is
already a finite from-C theorem (`type_I_p5_through_e_3AB_positive`). For
every prime p≥7, `|μ|≤2/n` on |κ|=1 is strictly stronger than
`|μ|≤L=(p-2)/(2p^2)` and closes the |κ|=1 half (`2/n < L` iff p≥7).
The |κ|=3 half separately requires
`χ_d((2p−1)μ+(p−2)ν)>−(p−2)/p`; only its particular/Wick term is
proved, while the even/odd δ remainder is open. Census slack at p=7 is
...
False. Current API correction: `e1_closed_general()` is the global gate and
is False; the old bounded AND is named only by
`e1_bounded_residual_split_closed()` and is True.
...
Current `e1_closed_general()` is False and represents the global E(1) gate.
Only `e1_bounded_residual_split_closed()` is True for the historical affine /
even-`k<=4p-2` residual split plus the two-level Type-I slice. Aut-Schur,
Gsum disj LB, and the cotangent pairing stay unused and False.
...
| 2 | `residual_ii_k_eq_4p_empty` / `multilevel_ND_k_ge_4p_proved` | Historical 2026-08-21 target, updated through 15.774: 15.734--15.754 close the first five shells and 15.752's higher band; 15.768--15.770 close the next two generic post-band layers and `p=23,t=9,10`; 15.771 closes `p=23,t=11,k=114`. Proposition 15.772 additionally closes `p=1 mod 4,p>=29,t=q-1,k=5p-3`. Proposition 15.773 closes `p>=29,t=q,k=5p-1`, and Proposition 15.774 closes `t=q+1,q+2,k=5p+1,5p+3` for every `p>=29`. With `q=(p-1)/2`, the exact frontier is critical `p=5,7`; `p=11,t>=3` (`k>=50`); `p=13,17,19,t>=5`; `p=23,t>=12`; `p=1 mod 4,p>=29,t>=q+3`; `p=3 mod 4,p>=31,t>=q+3`; and positive `p=7,z=7`, so the all-prime predicate remains False. |
...
gate and is False, while `e1_bounded_residual_split_closed()` alone records
the bounded True.
...
| Residual (ii), even \(k\ge4p\) | `residual_ii_k_eq_4p_empty=False` | Open globally. Propositions 15.734--15.754 close the first five shells and 15.752's higher band; 15.768--15.770 close the next two generic post-band layers and `p=23,t=9,10`; 15.771 closes `p=23,t=11,k=114`. Proposition 15.772 additionally closes `p=1 mod 4,p>=29,t=q-1,k=5p-3`. Proposition 15.773 closes `p>=29,t=q,k=5p-1` in both classes. Proposition 15.774 closes `p>=29,t=q+1,q+2,k=5p+1,5p+3`. With `q=(p-1)/2`, the exact frontier is critical `p=5,7`; `p=11,t>=3` (`k>=50`); `p=13,17,19,t>=5`; `p=23,t>=12`; `p=1 mod 4,p>=29,t>=q+3`; `p=3 mod 4,p>=31,t>=q+3`; and positive `p=7,z=7`. The all-prime legacy predicate therefore stays False. |
...
the `p=13,k=7` orbit decomposition: accumulate the
...
- 15.721--15.732 are the historical first-shell ladder: they identify the
  `|D|=p+1` endpoint normal form, its repair ensemble, and the exact-cycle
  method barrier. Proposition 15.733 subsequently excludes the
  `p=31,R=10` endpoint by simultaneous exact baseline coefficients.
- 15.734--15.735 supersede the shell ladder as the active large-prime gate.
  An isolated vertex can be transported to infinity, so `I=0` without any
  boundary-size assumption; exact coefficient offsets and the lift floor
  exclude `k in {4p,4p+2,4p+4}` for every prime `p>=13`.
- 15.736 gives the exhaustive finite sharp Boolean-quadratic catalog on
  `J(11,6)`. Proposition 15.737 combines that catalog with a binary quadratic
  moment to close the same three layers `k=44,46,48` at `p=11`. No external
  restriction-extension theorem is assumed.
- Combined, 15.734--15.735 and 15.737 close the first three residual shells
  for every `p>=11` and every boundary size. Proposition 15.751 closes the
  fourth shell for `p>=13`; 15.752--15.754 together close the fifth shell
  for every `p>=13`, and 15.752 also closes its stated higher band.
  Propositions 15.768--15.770 close the next two generic post-band layers
  and `p=23,t=9,10`; 15.771 adds `p=23,t=11,k=114`.
  With `q=(p-1)/2`, the exact remaining frontier is
  critical `p=5,7`; `p=11,t>=3` (`k>=50`); `p=13,17,19,t>=5`;
  `p=23,t>=12`; `p=1 mod 4,p>=29,t>=q+3`;
  `p=3 mod 4,p>=31,t>=q+3`; and positive `p=7,z=7`. Proposition 15.753 closes the p17/p19 fifth-shell
  ...
  15.746--15.749 classify and close `u=4`; 15.754 closes `u=6`.
...
| `src/e1_gmin_m4_prop15728.py` | Historical necessary Paley-hard direction profile at the `p=31,R=10` endpoint, later closed by 15.733 and subsumed by 15.734 |
...
| `src/e1_gmin_m4_prop15730.py` | All maximum repairs, complementary arcs, and exact two-colour line/direction census; no endpoint close |
| `src/e1_gmin_m4_prop15731.py` | Direct tangent-envelope gluing and coherent quadratic/cubic adjacent-repair transitions; no endpoint close |
| `src/e1_gmin_m4_prop15732.py` | Exact repair-cycle coboundary, rich-direction first jet, and failed natural phase bridges; no endpoint close |
| `src/e1_gmin_m4_prop15733.py` | Simultaneous exact-baseline contradiction closing the historical `p=31,R=10` endpoint |
| `src/e1_gmin_m4_prop15734.py` | Isolated-chart proof excluding critical `k=4p` for every boundary and every `p>=13` |
| `src/e1_gmin_m4_prop15735.py` | Isolated-chart extension excluding `k=4p+2,4p+4` for every boundary and every `p>=13` |
| `src/e1_gmin_m4_prop15736.py` | Exhaustive sharp Boolean-quadratic catalog on `J(11,6)` |
| `src/e1_gmin_m4_prop15737.py` | Binary quadratic moment close of `p=11`, `k=44,46,48` |
...
| `src/e1_gmin_m4_prop15741.py` | Common-graph moments, exact difference-Radon inverse, and `707+26C` energy identity |
| `src/e1_gmin_m4_prop15742.py` | Exhaustive six-bin row-energy certificate closing the generic p13 partition and `p=13,k=58` |
...
| `src/e1_gmin_m4_prop15751.py`, `tests/test_prop15751.py` | Dimension-free half-mean and fixed four-bit influence certificate closing the fourth shell for every `p>=13` |
| `src/e1_gmin_m4_prop15752.py`, `tests/test_prop15752.py` | Scaled-mass `p+9` theorem and isolated-chart replay closing its infinite fifth-and-higher-shell band |
| `src/e1_gmin_m4_prop15753.py`, `tests/test_prop15753.py` | All-19-model exact translated-cut and common-Parseval certificate closing the `p=17,19` fifth-shell endpoints |
| `src/e1_gmin_m4_prop15754.py`, `tests/test_prop15754.py` | Exact normalization, artifact manifest, seven-partition closure, corruption regressions, and global-open guards for the p13 endpoint |
| `src/e1_gmin_m4_prop15755.py`, `tests/test_prop15755.py`, `evidence/e1_gmin_m4_prop15755.json` | Full-cube dangerous-spike dichotomy, sharp affine/triangle endpoints, and sparse affine-alias obstruction |
| `src/e1_gmin_m4_prop15756.py`, `tests/test_prop15756.py`, `evidence/e1_gmin_m4_prop15756.json` | Arbitrary-boundary typed identity, vacuous Weil cap, and sharp two-parallel-line family |
...
| `src/e1_gmin_m4_prop15758.py`, `tests/test_prop15758.py`, `evidence/e1_gmin_m4_prop15758.json` | Sharp fixed-offset coefficient cancellation, two local rays, and scalar-Parseval barrier |
...
| `evidence/NOTE_2026-09-01_GENERIC_T3_INFLUENCE_CLOSE.md`, `evidence/e1_gmin_m4_prop15751.json` | Proposition 15.751 proof ledger and fixed-catalog evidence |
| `evidence/NOTE_2026-09-01_RESIDUAL_BAND_INFLUENCE_CLOSE.md`, `evidence/e1_gmin_m4_prop15752.json` | Proposition 15.752 infinite-band proof ledger and evidence |
| `evidence/NOTE_2026-09-01_P17_P19_FIFTH_SHELL_CLOSE.md`, `evidence/e1_gmin_m4_prop15753.json` | Proposition 15.753 endpoint proof ledger and 19 live one-worker replay records |
...
| `evidence/NOTE_2026-08-31_affine_unique_trisecant_reduction.md` | Full proof ledger for 15.729 and the retracted ceiling route |
| `evidence/NOTE_2026-08-31_endpoint_repair_ensemble.md` | Full proof ledger for the 15.730 complementary-repair ensemble |
| `evidence/NOTE_2026-08-31_endpoint_tangent_envelope_gluing.md` | Full proof ledger for the 15.731 envelope and swap-transition reduction |
| `evidence/NOTE_2026-08-31_repair_cycle_exactness.md` | Full proof ledger for the 15.732 exact-cycle and phase-bridge audit |
| `evidence/NOTE_2026-08-31_p31_simultaneous_baseline_close.md` | Symbolic 15.733 proof closing the historical `p=31` endpoint |
| `evidence/NOTE_2026-08-31_isolated_chart_k_eq_4p_close.md` | Full 15.734 isolated-chart critical-layer proof ledger |
| `evidence/NOTE_2026-08-31_first_three_residual_shells.md` | Full 15.735 proof ledger for the next two large-prime layers |
| `evidence/NOTE_2026-08-31_p11_sharp_boolean_quadratic_catalog.md` | Exact 15.736 finite-catalog certificate and audit |
| `evidence/NOTE_2026-08-31_p11_binary_quadratic_moment_close.md` | Full 15.737 proof ledger for the three `p=11` layers |
| `evidence/NOTE_2026-08-31_p13_mass14_boolean_catalog.md` | Exact 15.738 rank, height-four infeasibility, Boolean catalog, offsets, and selected-pair moments |
| `evidence/NOTE_2026-08-31_p13_exceptional_quartic_close.md` | Corrected 15.739 exceptional ledger, sign-safe quartic contradiction, and the surviving generic moment frontier |
| `evidence/NOTE_2026-08-31_p13_generic_translation_average.md` | Exact 15.740 six-variable translation-average certificate and the remaining common-graph gate |
| `evidence/NOTE_2026-08-31_p13_common_graph_moment_transform.md` | Exact 15.741 common-graph moment, difference-Radon, collision, and method-barrier gate |
| `evidence/NOTE_2026-08-31_p13_six_dilate_energy_close.md` | Exact 15.742 six-dilate row maxima and `667<707` common-energy contradiction |
| `evidence/NOTE_2026-08-31_p17_full_translated_cut_energy_close.md` | Exact 15.743 common-signed-total normalization, 698-cut row bounds, and `767<1211` contradiction |
...
| `evidence/p7_infinity7_positive_z7_global_semigroup_summary.json` | Compact 15.718 certificate |
| `evidence/p7_infinity7_positive_z7_projected_stabilization_summary.json` | Compact 15.719 certificate |
