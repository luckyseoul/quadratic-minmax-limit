...
> **Now (2026-08-18):** HEAD 15.524. **Three leftovers remain.** Type I: \(\sum_{x\neq y\neq0}\chi(x)\chi(y)\kappa\psi_\xi=3p\,\chi_p(-1)\chi(\xi)\) (15.523; fail: drop one pairing). Live \(\Delta_{\mathrm{conn}}\) unnamed. Floor: live \(Q_\tau\cdot D\) is a \(\mathbb Z[\sqrt{-2}]\) norm at p=5,7 (15.522). Fail: same in \(\mathbb Z[i]\) (48 not \(a^2+b^2\)). Residual (ii): leftover+s₊=2 at p=5 \(k=20\) empty for nF=0..7 (15.521 nF=4–6; 15.524 nF=7 with \(S\ge2\)). nF≥8 open. \(\phi_F\) not imported. \(L=\tfrac12\) is **not settled**.  
