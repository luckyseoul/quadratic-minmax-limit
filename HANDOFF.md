...
- **Prop 15.54 (moduli \(c\)-calculus + wedge + a-slice + deep refresh):** wedge \(G=\pm1/p\) proved; \(g_{\min}(c)\) + quadratic \(\mathrm{Tr}(G(c)^2)\) certified; a_min certs p=5,7; deep covers k=32..40 spike+Φ≥Φ(C) at p=5. `e1_gmin_cbound.json`, `e1_gmin_abound.json`, `e1_deep_spike_theory.json`. **Still OPEN:** Max+-free \(\mathrm{Tr}(G^2)\) for general \(p\); \(g_{\min}>T(p)\) ∀p≥5; uniform deep ND.
- **Residual to \(m_n\ge\Phi-2\Rightarrow L=\tfrac12\):** (1) \(g_{\min}>T(p)\) all primes \(p\ge5\); (2) deep non-tight ND. **F13/F16/F17** intact.
