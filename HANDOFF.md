...
> **Now (2026-08-17 evening):** Leftover 1 still **OPEN**. 15.453–15.483 shipped. The 4-point is not Cy=py-constant; TR \(L_{++}\) is. The p=5 \(Q_{\mathrm{eq}}\) mix recovers \(48/13\) and dies at p=7 (\(40/9\neq40/11\)). \(Q_\tau\) unnamed in \(p\). p=11 6-net — leave PID 712954. Do **not** import \(\phi_F\). Residual (ii) \(k\ge4p\) and Type I multi-level still OPEN. Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
