...
> **Now (2026-08-18):** HEAD 15.531–15.541. **Three leftovers remain.** Residual (ii): leftover+s₊=2 at p=5 \(k=20\) is empty for **all nF** (15.528); even \(k>4p\) with far stays open. Floor: 15.541 names \(c_{\mathrm{eq}}\) as the \(Q_{\mathrm{eq}}\)-Hoffman floor; live \(c=c_{\mathrm{eq}}\) only at p=5,7. Type I: 15.540 \(A_{1d}(r)\) on all \(r\), \(A_{\mathrm{free}}\) not a \(p\)-law, still den \(D\). \(\phi_F\) not imported. \(L=\tfrac12\) is **not settled**.  
