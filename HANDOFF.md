...
> **Now (2026-08-18):** **Three of four leftovers remain.** Lemma D is already True (do not unflip). Still OPEN: (1) floor \(\lambda_{\min}(\Phi)\ge6\) / name \(Q_\tau\) (15.505: affine-R Fourier names \(Q_R(r)/q^2=1/(\sin^2(\pi/p)\sin^2(\pi r/p))\); at p=5 this is \(16/5=Q_{\mathrm{NL}}\) and the 1D+R mixture is \(48/13\). Not a p-law: p=7 NL is not all R); (2) residual (ii) even \(k\ge4p\); (3) Type I multi-level. Do **not** import \(\phi_F\). Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
