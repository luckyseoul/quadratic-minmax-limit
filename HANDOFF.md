...
> **Now (2026-08-18):** HEAD 15.548. **Three leftovers remain.** Floor: 15.548 names the \(H_+\) 2-point \(G=(N/p)\chi\) and splits the true \(|\kappa|=3\) 4-linear as the Jacobi \(I_3=(1+\chi(\lambda))(1+\chi(1-\lambda))/4\); \(K_\lambda\) still unnamed, so \(16pA\) is not a p-law. Residual (ii): leftover+splus at p=5 \(k=22\) empty nF=0,3–8,14 (15.547). Type I: 15.546 p=7 mix \(109/2863<|T|\). \(\phi_F\) not imported. \(L=\tfrac12\) is **not settled**.  
