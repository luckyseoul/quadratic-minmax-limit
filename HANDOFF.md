...
> **Now (2026-08-18):** HEAD 15.531–15.534. **Three leftovers remain.** Residual (ii): leftover+s₊=2 at p=5 \(k=20\) is empty for **all nF** (15.528); even \(k>4p\) with far stays open. Floor: \(T_v\) free iff \(\chi(v)=-1\) (15.532); Aut_∞ involutions do not pair \(T_{\mathrm{ns}}\) (15.533). Type I: 15.529 \(\Delta_{\mathrm{conn}}\); 15.536 \(I_\Omega=p\,S(r/(1+r))\); 15.538 \(\Omega\) half-Gauss, \(A_{1d,\mathrm{dbl}}=-4p^3/(p-2)\), still den \(D\). \(\phi_F\) not imported. \(L=\tfrac12\) is **not settled**.  
