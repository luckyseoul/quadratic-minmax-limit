...
> **Now (2026-08-17 evening):** **Three of four leftovers remain.** Lemma D is already True (do not unflip). Still OPEN: (1) floor \(\lambda_{\min}(\Phi)\ge6\) / name \(Q_\tau\) (15.499: T-orbits free of size \(p^2\), \(n_T=c_{\mathrm{eq}}(p-1)\) only as a p=5,7 identity; \(\mathbb{F}_p^\times\) orbits are not all size \(p-1\). \(D\) unnamed); (2) residual (ii) even \(k\ge4p\); (3) Type I multi-level (15.495: \(\sigma_{k1}\equiv0\) on Max±; still \(G>T\) / \(|\mu|\le|L|\)). Do **not** import \(\phi_F\). Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
