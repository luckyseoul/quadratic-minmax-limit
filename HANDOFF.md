...
**Status date:** 2026-07-30 (Prop 15.62 typeA+wedge; L still OPEN)  
...
- **Prop 15.62 (typeA+wedge \(=6N\); \(Q=6N\|B\|_F^2+Q_4\); 2026-07-30):** **Proved:** for zero-diag \(B=P_+BP_+\), same-edge contrib \(=2N\|B\|_F^2\), wedge contrib \(=4N\|B\|_F^2\) (via \(\Sigma=2P_+\) on \(V_+\)), hence \(Q=6N\|B\|_F^2+Q_4\) with \(Q_4=Be^\top\mathrm{Gu}_{\mathrm{disj}}Be\) only. **Equivalence:** 16N \(\Leftrightarrow Q_4\le10N\|B\|_F^2\). **Certified** multi-seed p=3,5,7 (`e1_gmin_typeA_wedge.json`); p=3 saturates \(Q\equiv16N\) on whole space. Unrestricted \(\|\mathrm{Gu}_{\mathrm{disj}}\|_{\mathrm{op}}\) too large. **OPEN residual:** prove \(Q_4\le10N\|B\|_F^2\) for all primes \(p\ge5\) (image Rayleigh only). Bi-tight **not** yet closed for general \(p\).
- **Residual to \(m_n\ge\Phi-2\Rightarrow L=\tfrac12\):** (1) prove \(Q_4\le10N\|B\|_F^2\) / 16N for all \(p\ge5\) (closes bi-tight via Prop 15.55/61); (2) deep non-tight ND. **F13/F16–F18** intact.
