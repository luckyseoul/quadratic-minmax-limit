...
**Prop 15.28 (2026-07-27):** size-\(p\) Max-covers are tight (\(S_F\equiv1\) on \(\mathrm{Max}_{+}\)). Covering \(p\)-stars spike: \(S_F=p\) on some \(\mathrm{Max}_{-}\) \(\Rightarrow\Phi=\Phi+2p\) (60 at \(n=10\), all \(\Phi=21\); 390 at \(n=26\), MITM \(\Phi=75\)). At \(\infty\), covering leaves = half the affine lines of \(\mathrm{AG}(2,p)\). Full size-\(p\) census at \(n=10\): 405 covers, all \(\Phi\ge17\), **0 undercuts**. Card-min Max-covers do not undercut on certified range. See `evidence/E1_SIZE_P_MAXCOVER.md`. **Does not close E(1).**
