...
> **Now (2026-08-17 evening):** Leftover 1 still **OPEN**. 15.453–15.468 shipped. \(L_{ns}=\mu_+\mu_-\); Max+ χ-pairs named from \(Cy=py\); 3-point of \(1_D\) not constant; \(L_{++}\) still unnamed. p=11 6-net ETA \(\ge580\) h — leave PID 712954. Do **not** import \(\phi_F\). Residual (ii) \(k\ge4p\) and Type I multi-level still OPEN. Aut-Schur **false**. Live `e1` is the old AND. \(L=\tfrac12\) is **not settled**.  
